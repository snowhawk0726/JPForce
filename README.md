...
日本語では、複数の動作や処理を一続きの流れとして表現する場合、
文末以外の述語は 連用形 に変化します。

日本語ふぉーすでは、この日本語的な表現をそのままプログラムに用いることができます。
...
日本語ふぉーすでは、  
**連用形は述語の表記上の変化であり、意味的には終止形と同一の述語**  
として扱われます。
...
といった **連用形 → 終止形** の対応辞書を持ち、  
構文解析・評価の段階では、同一の述語として処理します。

これにより、日本語として自然な複文表現と、  
プログラミング言語としての一貫した処理モデルを両立しています。

#### 単文と複文

日本語文の中心となるものは**述語**で、その述語を１つ含む文を**単文**と呼びます。  
この単文が複数からなる文を**複文**と呼びます。  

日本語の複文には色々な種類ありますが、日本語ふぉーすでは、**連用形**で繋がる、連続した処理を表す複数の単文を**複文**と呼んでいます。
...
(2) １と２を足し、３を足す。(複文)
```
(2)では、２つの述語「足す」が連なり、１つ目は連用形「足し」、２つめは終止形「足す」となっています。

ふぉーすでは、この日本語の構造に合わせて、
**連用形で終わる述語を文の途中として解釈し、終止形の述語までを1つの複文として扱います。**

これにより、処理の流れを日本語の文章として自然に記述することが可能になります。
...
### 開発/実行環境(2026 年 ７ 月 現在)

- Swift: Swift 6.3.3
- xcode: Version 26.6.6
- MacOS: Tahoe 26.5.2
...
REPL には、以下の３つモードがあり、コマンド入力で切り替えます。
1. インタープリタ・モード  
インタープリタで、プログラムの実行・確認を行う。
1. VMモード  
プログラムを翻訳(compile)し、仮想マシン(VM)で実行・確認を行う。([翻訳器と仮想マシン](#翻訳器compilerと仮想マシンvm)参照)
1. 字句解析モード  
字句解析器(Lexer)で、入力文の字句解析し、結果を確認する。([字句解析器](#字句解析器lexer)参照) 

インタープリタで、複数行のプログラムを実行するには、まず、ファイルにテキストでプログラムを記述し、「書類」フォルダーに保存します。  
...
インタープリタでは、プログラムは以下の様に実行され結果が表示されます。
...
>> 関数であって、【「何もしない】
字句解析エラー： 」が見つかりません。
構文解析器が、3個のエラーを検出しました。
	式の解析で、「ILLEGAL("」が見つかりません")」に対応する構文解析方法が実装されていません。
	ブロック(【】)内で文の解析に失敗しました。
	関数で、「本体は、〜」の解析に失敗しました。
>>
```
...
日本語ふぉーす(JPForce)のREPLです。
>> 甲は、範囲【１から２以上】。
構文解析器が、2個のエラーを検出しました。
	範囲で、上限の形式が間違っています。(解析位置: 】)
	定義文「甲は、<式(値)>。」で、式の解釈に失敗しました。(解析位置: 】)
>>
```
...
REPLは、プログラムを、バイトコードに翻訳(コンパイル)し、仮想マシンで実行する環境を提供しています。  

注：  
コンパイラ・VMでは、インタープリタの一部機能が、未実装です。  
未実装の場合、以下の様に表示されます。  

```console
>> ファイル「test.txt」を実行する。
翻訳器が、エラーを検出しました。
	エラー: Label「ファイル「STRING("test.txt")」」は、翻訳不可(未実装)です。
```
...
注；仮想マシンとインタープリタは独立しており、スタック、辞書など環境は両者で共有していません。

##### 翻訳と実行

仮想マシン・モードでは、インタープリタ・モードと同様にプログラムを記述し、改行することにより、プログラムが翻訳(コンパイル)され実行されます。

注：翻訳に先立ち、定数解析を行います。詳細は、[定数解析](#定数解析)を参照してください。
...
翻訳の結果であるバイトコードは、逆アセンブル(disassemble)されたものが表示されます。
...
翻訳時に、関数の処理コードは、`翻訳済み関数`としてバイトコードが定数表に格納されます。  
定数表に`翻訳済み関数`がある場合は、バイトコードを逆アセンブル結果を表示します。
...
仮想マシン・モードでは、翻訳に先立ち**定数解析**を行います。  
定数解析は、入力の定数を解析し、計算が可能であれば、計算を進め結果を保持します。  
解析の結果、計算が完了する場合は、**計算結果を表示**します。
...
定数解析は、必要に応じてコマンドでオン/オフすることができます。

```
形式１： optimize       (定数解析のオン/オフを表示する。)
形式２： optimize off   (定数解析をオフにする。)
形式３： optimize on    (定数解析をオンにする。)
```

定数解析は**デフォルトでオン**です。  
コマンドを入力すると、以下の様に定数解析の**状態(on/off)を表示**します。
...
定数解析をオフにすると、定数解析なしに翻訳・実行が行われます。
...
翻訳結果：
0000 OpConstant 0	: 1(数値)
0003 OpConstant 1	: 3(数値)
0006 OpArrayConcat

実行結果: 配列であって、【要素が、1と、3】
入力: (配列であって、【要素が、1と、3】)
>> 
```

注：コンパイラ・定数解析の実装は未完のため、正しく機能しない場合があります。(2026年7月現在)
