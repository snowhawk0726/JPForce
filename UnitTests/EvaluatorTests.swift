...
final class EvaluatorTests: XCTestCase {
    ...
    func testTypeOperation() throws {
        let input = """
            自動車は、型であって、【
                型の要素が、【
                    ハンドルは、「右」。
                    切り替えるは、関数【ハンドルが「右」である場合【「ハンドル」に「左」を上書き】、それ以外は【「ハンドル」に「右」を上書き】】。
                】
    ...
    func testBuiltinFunctions() throws {
        let testPatterns: [(input: String, exptected: Any)] = [
            ...
            ("配列【１、２、３、４、５】が配列【１、３】を含む", false), // できない... ("配列【１、２、３、４、配列【１、３】】が配列【１、３】を含む", true),
            ...
            // フィルターする
            ("配列【１、２0、３、４0、５】を、関数【入力が数値、数値が10より大きい】でフィルターする。", [20,40]),
    ...
    func testOverwriteOperation() throws {
        let input = """
            行列は、配列。
            入れるは、関数で、入力が値、行列に値を追加し、行列に上書きする。
            出すは、関数で、行列の先頭の値(を )積み、行列が空でない場合、行列の残りを行列に上書きする。
            3を入れる。
            5を入れる。
            出す。
        """
    ...
    func testCompoundAssignmentOperations() throws {
        let testPatterns: [(input: String, expected: Any?)] = [
            ...
            ("xは１。xに1を足し代入。x", "仕様：<識別子>(を)<演算し>て代入する。"),
            ("""
            xは１。
            yは、関数【xに1を足して上書き。】
            yを実行する。
