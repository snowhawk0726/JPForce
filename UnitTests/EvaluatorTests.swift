...
final class EvaluatorTests: XCTestCase {
    ...
    func testTypeObject() throws {
        let input = """
            型であって、【
                初期化は、【行列は、配列【】】。
                入れるは、関数で、入力が値、行列に値を追加し、行列に代入する。
                出すは、関数で、行列の先頭の値を積み、行列が空でない場合、行列の残りを行列に代入する。
            】
        """
        print("テストパターン: \(input)")
        ...
        _ = try XCTUnwrap(first.rhsLiteral as? FunctionLiteral)
        _ = try XCTUnwrap(second.rhsLiteral as? FunctionLiteral)
    ...
    func testArrayAppendeds() throws {
        let testPatterns: [(input: String, expected: Int?)] = [
            ...
            ("配列【1, 2, 3】の1に、0を追加したものの1番目", 0),
            ("nは1。配列【1, 2, 3】のnに、0を追加したものの1番目", 0),
            ("nは1。aは配列【1, 2, 3】。aのnに、0を追加。aの1番目", 2),   // aには代入されない
        ]
        for test in testPatterns {
            print("テストパターン: \(test.input)")
            let evaluated = try XCTUnwrap(testEvaluator(test.input))
    ...
    func testArrayDeleteds() throws {
        let testPatterns: [(input: String, expected: Int?)] = [
            ...
            ("配列【1, 2, 3】から３番目を削除したものの後尾。", 3), // 削除できない場合は、元の配列を返す
    ...
    func testStackOperations() throws {
        let testPatterns: [(input: String, expected: Any?)] = [
            ...
            ("10と11と12を積む。甲に３個写す。空にする。甲の最後の値。", 12),
            ("13と14と15を積む。甲に３個得る。甲の最初の値。", 13),
            ("16を積む。甲は3個得たもの。甲", "スタックの値の数が不足しています。"),
            ("17を積む。甲に「数値」を得る。甲の値。", 17),
            ("「a」を積む。甲は「数値」を得たもの。甲。", nil),
            ("「b」と「c」と「d」を積む。甲に「数値」を3個得る。甲。", [nil,nil,nil]),
            ("18と19と20を積む。甲に「数値」を3個得る。甲の１番目の値。", 19),
    ...
    func testOuterIdentifiers() throws {
        let testPatterns: [(input: String, expected: Any?)] = [
            ...
            ("甲は配列【１、２、３】。乙は２。関数【乙は１。外部「甲」の外部『乙』に０を代入】を実行。甲。", [1,2,0]),
            ("甲は、辞書【1が「一」、2が「二」、3が「三」】。乙は２。関数【甲の外部「乙」は「弍」。】を実行。甲の2の値。", "弍"),
        ]
        for test in testPatterns {
            print("テストパターン: \(test.input)")
            let evaluated = try XCTUnwrap(testEvaluator(test.input))
    ...
    func testAssignToContainers() throws {
        let testPatterns: [(input: String, expected: Any?)] = [
            ...
            // 辞書への代入
            ("辞書【1が「一」、2が「二」、3が「三」】の2に「弍」を代入したものの2の値","弍"),
            ("辞書【1が「一」、2が「二」、3が「三」】の4に「四」を代入したものの4の値","四"),
            ("甲は、辞書【「い」が「一」、「ろ」が「二」、「は」が「三」】。甲の「は」に「参」を代入する。甲の「は」の値", "参"),
            ("甲は、辞書【真が1、偽が0】。甲の偽に2を代入する。甲の偽の値",2),
            ("甲は、辞書【1が「一」、2が「二」、3が「三」】。乙は２。関数【「弍」を甲の乙に代入する。】を実行する。甲の2の値", "弍"),
            // <配列>の<位置>は、<値>
