...
final class EvaluatorTests: XCTestCase {
    ...
    func testErrorHandling() throws {
        let testPatterns: [(input: String, exptected: String)] = [
            ...
            ("foobar。", "エラー：識別子『foobar』が定義されていない。"),
            ("『キー』は、関数【入力がx,x】。辞書【「名前」が「ふぉーす」】の『キー』の値。",
             "エラー：「関数」は、「辞書」の要素の索引(ハッシュキー)として使用できない。"),
            ("甲は１。甲を削除し、甲を表示する。", "エラー：識別子『甲』が定義されていない。"),
            ("それに1を足す。", "エラー：「足す」には、２つ以上の数値、文字列、配列の入力が必要。仕様：(〜と…)〜を足す。"),
            ("その型。", "エラー：「その」が指すオブジェクトが無い。(スタックが空)"),
            ("条件は、１。条件によって、「a」か「b」。", "エラー：「によって」の前には、条件式(真偽値)が必要。"),
            ("１と２を足すかによって、「a」か「b」。", "エラー：助詞「か」の前には、条件式(文)が必要。"),
    ...
    func testFunctionApplications() throws {
        let testPatterns: [(input: String, exptected: Any)] = [
            ...
            ("加算は、関数【入力がa「数値に」とb「数値を」で、aにbを足し、返す】。1と2を加算する。", "入力の格が異なる。入力の格：と"),
    ...
    func testInitializers() throws {
        let testPatterns: [(input: String, expected: Any?)] = [
            ...
            ("甲は型【初期化は【出力は「自身」。無を返す】】。甲【】の型", "期待しない出力。型：無"),
            ("甲は型【初期化は【出力は「自身」。無を返す】】。甲を生成するの型", "期待しない出力。型：無"),
            ("甲は型【初期化は【出力は「自身」。1を0で割る】】。甲【】の型", "期待しない出力。型：エラー"),
            ...
            ("甲は型【初期化は【出力は「自身か無」。１を返す】】。甲【】の型", "出力の型が異なる。実際の型：数値"),
            ("甲は型【初期化は【出力は「自身か無」。１】】。甲を生成するの型", "出力の型が異なる。実際の型：数値"),
            ("甲は型【初期化は【出力は「自身かエラー」】】。甲【】の型", "甲"),
            ("甲は型【初期化は【出力は「自身かエラー」。１を０で割る】】。甲【】の型", "1を0で割ることはできない。"),
            ("甲は型【初期化は【出力は「自身かエラー」。１を返す】】。甲【】の型", "出力の型が異なる。実際の型：数値"),
        ]
        for test in testPatterns {
            print("テストパターン: \(test.input)")
            if let evaluated = testEvaluator(test.input) {
    ...
    func testConformanceErrors() throws {
        let testPatterns: [(input: String, expected: Any?)] = [
            // 規約の定義
            ("甲は型【規約は乙。丙は関数【出力は「」】】。甲【】", "準拠する規約「乙」が見つからない。"),
            // 条項(識別子)の定義
            ("甲は規約【乙は関数【出力は「数値」】】。丙は型【規約は甲。丁は関数【出力は「」】】。丙【】",               "規約に準拠するためには、「乙(型：関数)」の定義が必要。"),
            ("甲は規約【乙は関数【出力は「数値」】】。丙は型【規約は甲。乙は算出【出力は「数値」】】。丙【】",             "規約に準拠するためには、「乙」を、型「関数」で定義する必要がある。"),
            ("甲は規約【乙は関数【入力がa】】。丙は型【規約は甲。乙は関数【】】。丙【】",                           "規約に準拠するためには、「乙」の入力定義に1個の引数が必要。"),
            ("甲は規約【乙は算出【設定が、【出力は「数値」】】】。丙は型【規約は甲。乙は算出【入力はa「数値」】】。丙【】",  "規約に準拠するためには、「乙」に「設定」の定義が必要。"),
            ("甲は規約【乙は算出【出力は「数値」】】。丙は型【規約は甲。乙は算出【設定が、【入力はa「数値」】】】。丙【】",  "規約に準拠するためには、「乙」に「取得」の定義が必要。"),
            ("甲は規約【乙は算出【設定が【入力はa「数値」】。取得が【入力はa「文字列」】】】。丙は型【規約は甲。乙は算出【】】。丙【】",               "規約に準拠するためには、「乙」に「設定と取得」の定義が必要。"),
            ("甲は規約【乙は算出【設定が【入力はa「数値」】。取得が【入力はa「文字列」】】】。丙は型【規約は甲。乙は算出【入力はa「文字列」】】。丙【】", "規約に準拠するためには、「乙」に「設定」の定義が必要。"),
            // 入力定義
            ("甲は規約【乙は関数【出力は「数値」】】。丙は型【規約は甲。乙は関数【入力はa「数値」】】。丙【】",    "「乙」には、入力定義は不要。"),
            ("甲は規約【乙は算出【出力は「数値」】】。丙は型【規約は甲。乙は算出【入力はa「数値」】】。丙【】",     "「乙」には、入力定義は不要。"),
            ("甲は規約【乙は関数【入力はaとb】】。丙は型【規約は甲。乙は関数【出力は「」】】。丙【】", "規約に準拠するためには、「乙」の入力定義に2個の引数が必要。"),
            ("甲は規約【乙は関数【入力はaとb】】。丙は型【規約は甲。乙は関数【入力はaとc】】。丙【】", "規約に準拠するためには、「乙」に引数「b」が必要。"),
            ("甲は規約【乙は関数【入力はa「数値に」とb「数値を」】】。丙は型【規約は甲。乙は関数【入力はa「数値に」とb「文字列を」】】。丙【】", "規約に準拠するためには、「乙」に入力形式「数値を」が必要。"),
            ("甲は規約【乙は関数【入力がa「数値と…」とb「数値を」。】】。丙は型【規約は甲。乙は関数【入力はa「数値と」とb「数値を」】】。丙【】", "規約に準拠するためには、「乙」に入力形式「数値と…」が必要。"),
            // 出力定義
            ("甲は規約【乙は関数【出力は「数値」】】。丙は型【規約は甲。乙は関数【出力は「文字列」】】。丙【】",            "規約に準拠するためには、「乙」に出力型「数値」が必要。"),
            ("甲は規約【乙は算出【設定が【出力は「数値」】】】。丙は型【規約は甲。乙は算出【設定が【出力は「文字列」】】】。丙【】",            "規約に準拠するためには、「乙」に出力型「数値」が必要。"),
            ("甲は規約【乙は関数【出力は「数値」と「文字列」】】。丙は型【規約は甲。乙は関数【出力は「文字列」】】。丙【】",  "規約に準拠するためには、「乙」の出力定義に2個の型が必要。"),
            ("甲は規約【乙は関数【入力がa】】。丙は型【規約は甲。乙は関数【入力がa。出力が「数値」】】。丙【】",           "「乙」には、出力定義は不要。"),
            ("甲は規約【初期化は、【入力がa。出力が「自身か無」】】。丙は型【規約は甲。初期化は、【入力がa。出力が「自身」】】。丙【】",           "規約に準拠するためには、「初期化」に出力型「自身か無」が必要。"),
        ]
        for test in testPatterns {
            print("テストパターン: \(test.input)")
            if let evaluated = testEvaluator(test.input) {
    ...
    func testStackOperations() throws {
        let testPatterns: [(input: String, expected: Any?)] = [
            ...
            ("2を積む。甲に得る。甲の値。", 2),
            ("3を積み、捨てる。甲は写したもの。甲。", "スタックの値の数が不足している。"),
            ...
            ("16を積む。甲は3個得たもの。甲", "スタックの値の数が不足している。"),
