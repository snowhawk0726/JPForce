...
| 05  | OpPop            | -                    | -                     | スタックのオブジェクトを１つ捨てる(未使用) |
...
| 10  | OpArrayConst     | 要素数(2)             | -                     | 指定の要素数を持つ配列をスタックに積む(要素はスタックから得る) |
| 11  | OpDictionaryConst| 要素数(2)             | -                     | 指定の要素数を持つ辞書をスタックに積む(要素はスタックから得る) |
...
| 23  | OpPullConst      | 要素数(1)             | -                     | 指定の要素数をスタックから得る |
| 24  | OpPull           | -                    | -                     | スタックに積まれた要素数をスタックから得る |
| 25  | OpDuplicateConst | 要素数(1)             | -                     | 指定の要素数をスタックからコピーする |
| 26  | OpDuplicate      | -                    | -                     | スタックに積まれた要素数をスタックからコピーする |
| 27  | OpDropConst      | 要素数(1)             | -                     | 指定の要素数をスタックから捨てる |
| 28  | OpDrop           | -                    | -                     | スタックに積まれた要素数をスタックから捨てる |
| 29  | OpArray          | -                    | -                     | スタックに積まれた要素数を持つ配列をスタックに積む |
| 30  | OpMapProperty    | 属性インデックス(1)     | -                     | スタックから得た配列の各オブジェクトの(指定の)属性を取得し、結果を配列にしてスタックに積む(配列内のオブジェクトをその属性に写像する) |
| 31  | OpRangeConst     | 要素数(1)             | -                     | 指定の要素数を持つ範囲オブジェクトをスタックに積む |
| 32  | OpArrayConcat    | -                    | -                     | スタックから２つの要素を得て、配列化して連結する |
| 33  | OpComparisonConst| 比較種別インデックス(1) | -                     | 指定の比較種別のインデックスを数値としてスタックに積む |
| 34  | OpArrayRepeat    | -                    | -                     | スタックから個数と要素を得て、指定個数の要素配列を作成し、スタックに積む |

以下に、命令語の補足をします。

### 格、述語、属性インデックスをオペランドに持つ命令語

`OpPhrase`、`OpPredicate`、`OpGetProperty`、`OpMapProperty`、`OpComparisonConst`がアクセスするデータは、内部的に静的な配列または列挙として実装しており、それぞれ、格、述語(識別子名)、属性(識別子名)、および比較種別が翻訳時に配列または列挙のインデックスに変換されます。  

VM は、これらのインデックスにより、処理を取り出し実行、または値を取り出し処理します。

`OpPhrase`は、インデックスから得た`格`を、スタックに積まれた値と合わせて動的に`句`(Phrase)を生成し、スタックに積みます。

`OpComparisonConst`は、範囲オブジェクトを生成するために**比較種別インデックス**から種別値(注)を生成し、スタックに積みます。

注：サポートする**比較種別**は、**以上**(1)、**未満**(2)、**以下**(3)です。
...
`OpArrayConst`、`OpDictionaryConst`等は、スタックに積まれた要素数分のオブジェクトを要素として、それぞれ`配列`、`辞書`オブジェクトを生成し、スタックに積みます。  
扱える要素数は最大 65,535 個です。  
ただし、`辞書`は、`キー`と`値`で１つの要素であるため、実際の`辞書`の要素数は指定の半分になります。

`OpArray`の様に命令語に **Const** が付かないものは、オペランドではなくスタックから要素数を得ます。(実行時に要素数が確定し、オペランドに定数指定できない場合これらが使われます。)

`OpRangeConst`は、数値オブジェクトと比較種別をペアとしたデータを指定個数スタックから取得し、範囲オブジェクトを生成します。個数は、上下限を含む場合は2、上限/下限の一方である場合は1が指定されます。  
比較種別は、`OpComparisonConst`を使って生成されます。
...
`OpGenitive`は、スタックの２つのオブジェクトから属格の値を得ます。
