...
let insertArrayUsage =      JpfError("仕様：〜(を)<配列>の〜に挿入する。または、<配列>の〜に〜を挿入する。")
let appendDictionaryUsage = JpfError("仕様：〜が〜(を)〜に追加する。または、〜(に)〜が〜を追加する。")
let removeUsage =           JpfError("仕様：(〜の)〜を削除する。")
let removeUsage2 =          JpfError("仕様：(〜の)〜を削除する。または、〜から〜を削除する。")
let removeUsage3 =          JpfError("仕様：〜から最初の/先頭の/全ての〜を削除する。")
...
let sortUsage =             JpfError("仕様：<オブジェクト>を<関数>で並べ替える。または、<オブジェクト>を(昇順に/降順に）並べ替える。")
let reverseUsage =          JpfError("仕様：<配列、文字列>を逆順にする。")
let pullDupUsage =          JpfError("仕様：(<識別子>と…)(<識別子>に）(数値を/中身を)(<数値>個)")
let assignUsage =           JpfError("仕様：〜(を)<識別子>に代入する。または、<識別子>に〜を代入する。")
let compoundAssignUsage =   JpfError("仕様：<識別子>(に)<計算し>て代入する。")
let assignArrayUsage =      JpfError("仕様：〜(を)<配列>の<位置>に代入する。または、<配列>の<位置>に〜を代入する。")
