...
final class Environment {
    ...
    /// 辞書から、指定Identifierに対応する値を取り出す。
    func value(for identifier: Identifier) -> JpfObject? {
        identifier.isOuter ?
            outer?[identifier.value] :
            self[identifier.value]
    }
    /// 指定がJpfIdentifierである場合、辞書から値を取り出す。
    func resolvedValue(for target: JpfObject?) -> JpfObject? {
        if let ident = target as? JpfIdentifier {
            return ident.isOuter ? outer?[ident.value] : self[ident.value]
        }
        return target
    }
    /// 識別子を辞書に代入する。
    ...
    ///  2) 外部に識別子があれば、外部に上書き
    ///  3) それ以外は、新規ローカル代入
    func assign(ident: Identifier, value: JpfObject) throws {
        // 1.外部指定代入
        if ident.isOuter {
            guard let outer, outer.contains(ident.value) else {
                throw outerUndefinedIdentifier(ident.value)
            }
            outer[ident.value] = value
            return
        }
        // 2.暗黙代入
        // 1) 上書き代入
        if contains(ident.value) {
            self[ident.value] = value
            return
        }
        // 2) 暗黙の外部代入
        if let outer, outer.contains(ident.value) {
            outer[ident.value] = value
            return
        }
        // 3) 新規代入
    ...
    var unwrappedValue: JpfObject? {
        guard let value = unwrappedPeek else {return nil}
        drop()
        return value
    }
