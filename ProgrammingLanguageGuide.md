...
さらに、述語`代入`(assign)によって、辞書の指定キーに値を代入(更新または追加)することが可能です。  
...
**形式１**では、<条件式>が真である間、<実行処理>を反復します。(他のプログラム言語の while 文に相当)  
...
三点リーダを指定した場合を覗き、引数の数もチェックします。引数の数が、指定した識別子の数に満たない場合はエラーになります。

注：関数が多重定義されている場合、上記のチェックによるエラーは検出されません。([関数の多重定義](#関数の多重定義function-overload)を参照)。  
...
`カウンター`は、型「計数」から生成したインスタンスで、３つの利用可能要素を持ちます。

(1)では、算出「増やす」の取得(形式２）を実行します。引数が無いため、１番目の算出が実行され、`カウント値`を１にします。  
`カウンター`の要素「値」は`算出`であり(形式１）、実行することにより、`カウント値`の値を返します。

(2)では、算出「増やす」の引数として`5個`が渡され(形式３）、２番目の算出が実行され、(1)の`カウント値`に５が足され、`カウント値`は６になります。(注：`個`は[助詞](#特殊な助詞)。)

(3)では、算出「リセット」が実行され、`カウント値`は０になります。

注：`リセット`では、外部の`カウント値`に 0 を`代入`しています。これを`カウント値は０`としてしまうと、新たな識別子「カウント値」が`算出`内の辞書に定義されてしまいます。

また、`外部「カウント値」は0`としてしまうと、`外部「カウント値」`は識別子でないため、[`定義文`](#定義文define-statement)とはならず、`カウント値`の値と、０がスタックに格納されます。)
...
注３：[代入文](#代入文assignment-statement---要素への代入設定)でも代入することができます。
...
注１：いずれも<値>を指定キーに代入した<辞書>を返します。  
注２：[代入文](#代入文assignment-statement---要素への代入設定)でも代入することができます。
...
`関数`や`算出` 内で、呼び出し元の識別子や、インスタンスのプロパティに値を代入することができます。

ローカルに同名の識別子が無く、呼び元やインスタンスの識別子を指定し代入すると、値が代入(上書き)されます。
同名の識別子がある場合、`外部`を指定することにより、代入可能です。
...
`算出`の定義内容を表示するためには、述語`識別子`を使います。(以下の例(2)を参照)
...
また、(3)のように、エスケープ文字`『』`で`識別子`を表示する場合は、`加える`は実行されません。
...
述語`ファイル`(file)により、ファイルにアクセスすることが可能です。ファイルに対し可能な処理は、以下のとおりです。
...
述語`ファイル一覧`(files)により、ファイルの一覧をファイル名(文字列)の`配列`として返します。
...
[配列要素のアクセス](#配列要素のアクセス)で、[`代入`](#配列に代入)時に配列の位置を指定します。  
指定する位置は、`数値`、もしくは数値を値に持つ`識別子`です。

キーに識別子を指定する場合、識別子は二重かぎ括弧`『』`で囲います。  
[`外部`(outer)](#外部outer)の識別子を指定する場合は、`『外部「<識別子>』`または`「外部『<識別子>』」`の形式で指定します。

#### `キー`(Key)

[辞書要素のアクセス](#辞書要素のアクセス)で、[`代入`](#辞書に代入)時に辞書のキーを指定します。  
指定するキーは、`文字列`、`数値`、`真偽値`、もしくは、これらを値に持つ`識別子`です。
...
[定義内容の表示](#定義内容の表示)を行う際、表示する`識別子`を指定します。  
指定する`識別子`は、二重かぎ括弧`『』`またはかぎ括弧`「」`で囲います。
...
関数の呼び元や、メソッドの属する型のインスタンス等(ローカルの外側の辞書)の識別子をアクセスする際に用います。

通常、外側の識別子は、同名の識別子がローカルに存在しない場合に、参照・変更が可能ですが、`外部`で指定することにより、直接参照・変更が可能になります。  
ただし、`外部`指定しても、定義文や`代入`による定義はできません。(既存の識別子が対象)
...
`代入`については、[関数の呼び元への代入](#関数の呼び元への代入)を参照。
...
[再定義可能な述語](#再定義可能な述語)で、予約語である述語を再定義または多重定義した際、元の述語を当形式で表します。
...
これらの述語は、再定義をする際に、以下の形式で元の述語として使用できます。
...
算出内の足し算には`予約語「足す」`を使います。
(注：この場合、述語を連用形にすることはできません。)
...
「再定義可能な述語」を副詞`さらに`を使って定義することにより、述語を[多重定義](#関数の多重定義function-overloading)(拡張)することができます。

```
形式：『<(再定義可能な)述語>』は、さらに、<関数定義>
```

この場合、呼び出す引数に対し、すべての拡張した定義が一致しない場合は、元の述語が呼び出されます。
...
『引く』は、座標 a から座標 b を引く`算出`として、多重定義されています。  
算出内の引き算は`予約語「引く」`を使います。(この場合、述語は連体形にできません。)

(1)では、当定義により、座標 A から座標 B を引いた値が、座標として返り、結果の配列が得られます。  
(2)では、座標どうしの引き算でないため、既定の述語`引く`が実行されます。

## 糖衣構文(Syntax Sugar)

述語による記述は、わかりやすくなる一方、冗長になることがあります。  
...
他のプログラミング言語では、この様な引数の形式を「キーワード引数(keyword argumant)」と呼んでいます。
...
(1)では、「数値」の指定に対し、「文字列」を渡しているためエラーとなります。(格`に`・`を`はチェックしない。)  
...
入力識別子`y`と`z`は、既定値を持つため省略可能です。((1)〜(3))  
...
### 代入文(Assignment Statement) - 要素への代入/設定

配列や辞書に値を代入するためには、述語`代入`(assign)を使い、`arrayの位置1に、3を代入する`の様に記述します。([配列への代入](#配列に代入)、[辞書への代入](#辞書に代入)を参照)  
また、型やインスタンスの要素に値を設定するためには、述語`設定`(set)を使い、`インスタンスの要素「メンバ」に値を設定する`の様に記述します。([設定(Set)参照](#設定set))

以下の様な要素への代入文により、これらを代替し、簡易に記述することができます。
...
この構文を評価・実行すると、述語の`代入`/`設定`と同様に、<要素>に<値>を代入したオブジェクトが返ります。

そして、<オブジェクト>が識別子で、辞書にある場合は、結果のオブジェクトが辞書に登録されます。  
また、識別子が、ローカルに無く外側の辞書（関数の呼び元等）にある場合、結果のオブジェクトは、外側の辞書に登録されます。  
(辞書に登録できない場合は、スタックに格納されます。)

注：外側とローカル辞書の両方に同じ識別子がある場合、代入結果は、ローカル辞書に登録されます。

外側の識別子を指定するためには、[述語`外部`](#外部outer)を使用します。
...
(1)では、辞書リテラルのキー 2 に「弍」を代入した結果が、辞書として返ります(スタックに格納される)。キー２の値は「弍」に変更されています。  
(2)では、辞書リテラルのキー 4 に「四」を代入した結果が、辞書として返ります(スタックに格納される)。キー 4、値２が追加されています。  
(3)では、キー「は」の値が「三」から「参」に変更されたオブジェクト「甲」が辞書が格納されています。  
(4)では、キー「偽」の値が０から 2 に変更されたオブジェクト「甲」が辞書が格納されています。
...
(1)では、関数内の代入文で、(ローカル辞書の外にある)呼び元の「甲」に代入結果が上書きされます。  
(2)でも同様に、呼び元の「甲」に代入結果が上書きされます。
...
関数リテラルは、辞書「甲」のキー「乙」(外部)に値「弍」を代入し、結果が呼び元の「甲」に代入されます。
