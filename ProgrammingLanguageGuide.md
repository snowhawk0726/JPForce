...
ただし、識別子を`甲「と…」`の様に可変長に指定した場合[(入力チェックを参照)](#入力チェック)、既定値を持つことはできません。　　

また、既定値を指定した識別子と、指定していない識別子が混在する場合、既定値を持たない識別子を先に定義してください。  
(引数は、先頭から割り当てるため、後ろの識別子に正しく値を割り当てることができなくなります。)
...
例では、３つの識別子が定義されています。識別子`呼びかけ`は、規定値「こんにちは」を持ちます。  
`相手１`は、可変長識別子であり、規定値を指定することはできません。
...
実行時に入力(引数の型と格)をチェックすることが可能です。チェックするには、入力の識別子にかぎ括弧`「`と`」`を付けて、以下の様に記述します。
...
<出力定義>として、型名を助詞`と`を使い、出力する順番に必要個並べることにより、実行時に関数の出力(返り値の型)をチェックします。  
...
さらに、`関数`、`算出`(`設定`および`取得`)では、必要に応じて、その入出力形式を規定することができます。
...
また、`関数`と`算出`で、入力形式の規定が不要である場合は、形式１、２の様にそれぞれの<型名>だけ記述します。  
