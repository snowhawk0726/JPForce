...
```
範囲【1に1を足すから、100を10で割るまで】(範囲【2から、10まで】)
```

ただし、当該**単文**に、範囲の**境界を表す助詞**を重複して記述することはできません。  

*例：*

```
範囲【10から1を引くから、10に1を足すまで】
(エラー：範囲式の単文には、引数に境界助詞を持つ述語「引く」を使用できません。)
```

例では、下限助詞`から`を引数に持つ述語`引く`が単文中で使用されているため、上記**エラー**になります。
...
範囲を表す助詞`以上`、`以下`、`未満`をよび記号`〜`を用いることにより、予約語`範囲`を使わずに**範囲リテラル**を記述できます。
...
**<式>** は、**「数値」** または **「識別子」** です。  
予約語`範囲`では、式の組み合わせが可能ですが、**省略記法では使えません**。
