...
*出力チェックのエラー例：*
```
(7) 関数【出力は「数値」。「い」】を実行。(エラー：出力の型が異なる。実際の型：文字列）
(8) 関数【出力は「数値」。１を０で割る】を実行。(エラー：出力の型が異なる。実際の型：エラー）
(9) 関数【出力は「数値」と「文字列」。1】を実行。(エラー：出力の数が足りていない。必要数：2)
(10) 関数【出力は「」。1】を実行。(エラー：期待しない出力。型：数値)
```

(8)の例では、(6)と異なり、**エラー型**をチェックしていないため、**「出力の型が異なる」** というエラーになります。  
...
(1) 得たものが無である。(真)
(2) それが無である。(偽)
(3) それに1を足す。(エラー：「足す」には、２つ以上の数値、文字列、配列の入力が必要。仕様：(〜と…)〜を足す。)
```
...
```
その値。(エラー：「その」が指すオブジェクトが無い。(スタックが空))
```
...
```
配列【1,2,3】。その数。(3)
配列【1,2,3】。その1番目。(「1」は属性名ではない。(解析位置: その))
１。その負数。(「負数」は属性名ではない。(解析位置: その))
```
