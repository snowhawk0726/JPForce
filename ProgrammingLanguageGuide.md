...
式は、文中で述語とその引数（目的語や補語など）を評価し、値を生成する単位です。  
文は、1個以上の式から構成されます。

ふぉーすでは、C言語などに見られる記号中心の数式  
（例：a = b * 2）は扱いません。  
代わりに、日本語の述語と助詞による表現を式の基本としています。

出力する値の種類により、以下のように呼び分けます。
-	数値 → 算術式  
-	文字列 → 文字式  
-	真偽値 → 条件式/論理式  
...
注： 上記はいずれも複数の式から成り立っています。  
ふぉーすにおいて、引数`aが`や述語`大きい`等が１つの式となります。

### 文の種類(単文と複文)

日本語文の中心となるものは**述語**です。
ふぉーすでは、この述語に基づいて文を分類します。
...
単文は、1 つの処理または判断を表す最小の文です。

#### 複文

複数の単文が連続し、1つの文として記述されるものを**複文**と呼びます。

日本語ふぉーすでは、日本語の文法構造に倣い、連用形で終わる述語を文の途中と解釈し、終止形の述語までを 1つの複文として扱います。
...
複文は、処理の流れを日本語の文章として自然に記述するための仕組みです。

### 値(Literal)とオブジェクト(Object)

プログラムに書かれる１や「いろは」などの値を表現する書式を **リテラル(literal)** と言います。  
リテラルは、コンピュータ内で解析・評価されて **オブジェクト(object)** というデータとして扱われます。このオブジェクトは、その振る舞いを決める**型**を持ちます。
...
予約語は、言語が定義する**組み込みの原語**です。
予約語には、以下があります。

1. 主に定義文で使用する「`配列`、`辞書`、`列挙`、`範囲`、`関数`、`型`、`規約`、`算出`」
1. 値(literal)として使用する「`真`、`偽`、`無`」
1. 制御文で使用する「`場合`、`反復`、`間`、`それ以外`」
1. 述語(名詞、動詞、形容詞）として使用する「`足す`、`引く`、`掛ける`、`割る`、`負数`、`等しい`、`小さい`、`大きい`、`ある`、`ない`、`返す`、`返る`、`実行`、`する`、`追加`、`挿入`、`削除`、`繰り返す`、`中止`、`継続`、`写像`、`まとめる`、`絞り込む`、`並び替える`、`含む`、`積む`、`捨てる`、`得る`、`写す`、`代入`、`入れ替える`、`表示`、`改行`、`読む`、`生成`、`設定`、`逆順`、`利用可能`、`初期化`、`識別子一覧`、`ファイル一覧`」
1. その他「`正`、`負`、`入力`、`空`、`かつ`、`または`、`および`、`もの`、`こと`、`か`、`よって`、`ファイル`、`識別子`、`要素`、`外部`」
...
助詞`から`と助詞`を`を、述語`引く`でチェックしているため、どちらの語順でも90という結果になります。

#### 特殊な助詞

また、特殊な助詞として、個数を表す`個`も同様に助詞として扱います。([数値に続く文字列](#数値に続く文字列)参照)  
...
識別子は、**文字**、**数字**、**記号**を含む語で表し、プログラム文中で、**名詞**、**動詞**、**形容詞**として使用します。
...
また、「中身」や「文字列」の様な[属性を表す語](#値literalとオブジェクトobject)と同じ識別子を使用することが可能です。  
...
プログラムを記述するために使用する記号は以下のとおりです。

- ブロック(`【】`)  
  墨付き括弧`【】`で括られた**複数の文**を、制御文や、関数等の定義において、ひとまとまりの「**ブロック**」として扱います。  
  ブロックの始まり`【`から終わり`】`までを１文で記述できる場合、`【】`は省略できます。    
  １文とは、定義文や述語文の**単文および複文**のことです。
- 句読点( `、` `。`)  
...
- マイナス(`ー`)  
  [数値](#数値number)において、負数であることを示します。  
  数値計算の演算子としては、使えません。
- 波線(`〜`)  
  「1〜」、「1〜10」の様に**数値の範囲**を表します。([範囲](#範囲range)参照)  
- タブ(`\t`)、空白(` `)、改行(`\n`)  
...
- 識別子（`『』`)  
  ...
  識別子は、定義時に終止形として登録されます。  
  文中では連用形などの活用形で記述することができ、評価時に対応する識別子が参照されます。  
- 予約語（`《》`)  
  二重山括弧`《》`で括られた文字列を、`予約語`として扱います。  
  通常、述語や語は名前解決により参照されますが、**再定義**が存在する場合など、  
	言語が定義する原語を明示的に指定する必要がある場合に《》を用います。  
	**再定義**が存在しない場合、通常表記と《》表記は同一の振る舞いを示します。  
	《》内の語は、活用形で記述された場合でも、終止形として解釈されます。  
  (再定義については、[予約語の多重定義拡張](#予約語の多重定義拡張)を参照)
- エスケープ文字(`\\`)  
...
#### 代替記号  
  以下の記号は、字句解析時に同一の記号として扱われます。(推奨は[上記記号](#記号symbol)です。)  
- `()`(丸括弧) : 全角丸括弧`（）`
- `«»`(ギュメ) : 二重山括弧`《》`
- `.`(半角ピリオド) : 句点`。`
- `．`(全角ピリオド) : 句点`。`
...
(日本語ふぉーすには、一般的な言語の`.`(ピリオド演算子)、`*`または`-`(算術演算子)はありません。既定記号の代替として使用されます。)

### 連用形(語尾変化)

語尾が変化する`予約語`および`識別子(動詞、形容詞)`は、**連用形**での使用が可能です。

連用形は、**複文**において文の途中に置かれる**述語**を表すための、表記上の変化です。  
意味的には、**終止形**と同一の述語として扱われます。

ふぉーすでは、連用形を終止形に正規化するため、以下の辞書を用いて述語を特定します。
...
**注：**  
辞書には、連用形だけでなく未然形の語尾も含みます。これは、述語`ない`が未然形に接続するためです。
...
式の評価結果として生成されたオブジェクトは、スタックに格納されます。  
**述語は**、これらのオブジェクトを引数としてスタックから取り出し、処理を行った結果を、再度スタックに出力します。  
...
句以外のオブジェクトに対して、これらを適用した場合、「<オブジェクト>の中身」はオブジェクトそのものが返り、「オブジェクト>の格」は`無`が返ります。  

また、句は、オブジェクトなので、[オブジェクトに対する属性の取得や判定](#値literalとオブジェクトobject)が使用可能です。

### 識別子の有効範囲(Scope)

識別子は、その値（オブジェクト）とともに「辞書（環境またはシンボルテーブル）」に登録されます。辞書は以下の階層（スコープ）で管理され、**各スコープ内で識別子はユニーク**です。

- 大域スコープ（最上位）  
- 型スコープ（型定義の静的要素・型メソッドなど）  
- インスタンススコープ（生成されたインスタンスの要素・メソッドなど）  
- 列挙スコープ（列挙定義・列挙子と関連値など）  
- 関数スコープ（パラメータ・ローカル変数・自由変数の捕捉）  
- 算出スコープ（取得/設定/派生ロジックの局所識別子）  

識別子の解決は、原則として「内側から外側へ」の順に探索します。つまり、現在スコープ → 親スコープ → … → 大域スコープの順で解決されます。

#### 再定義と上書き（シャドーイング）

- 有効範囲内で同名の識別子に再代入・再定義すると、同一スコープ内の既存の値や定義が上書きされます。
- 外側のスコープに同名の識別子が存在しても、内側スコープで同名を定義すれば、以降その参照は内側スコープの識別子（ローカル辞書）を指します（シャドーイング）。

#### スコープをまたぐ参照（自由変数）

- 関数呼び出しやインスタンス生成後でも、呼び出し元（または同一インスタンス内）の識別子を参照できます。関数は必要に応じて自由変数を捕捉し、実行時に正しいスコープ連鎖で解決されます。
- ただし、関数（または算出）内で同名の識別子を定義した場合、その識別子はローカル辞書に登録され、以後の参照はローカルを指します。

#### 呼び元の識別子の更新

- 述語「代入」 や 代入文 を用いると、呼び元が保持する識別子の値を変更できます。これにより、関数の外側やインスタンス外部にある既存の識別子へ副作用を及ぼすことが可能です。
- この操作は、対象識別子がどのスコープに存在するかに応じて、そのスコープの辞書へ反映されます。

#### 外部参照（予約語「外部」）

- ローカルで同名の識別子が存在し、外側の識別子を明示的に参照したい場合は、予約語「外部」を使用します（例：外部「x」）。

これにより、シャドーイングの影響を回避して、外側スコープの識別子にアクセスできます。詳細は [関数の呼び元への代入](#関数の呼び元への代入) を参照してください。

## 値(Literal)と型(Type)

### オブジェクトの種類

ふぉーすでは、オブジェクトの型として、`数値`,`文字列`,`真偽値`,`無`,`エラー`,`配列`,`辞書`,`範囲`,`列挙`,`関数`,`型`,`規約`,`算出`があります。

以下に、共通する定義形式について説明し、その後で個別の型について説明します。

### 複合オブジェクトの定義形式（共通構文）

`数値`,`文字列`,`真偽値`,`無`,`エラー`は単純なリテラル値を表す基本オブジェクト、  
それ以外の、`配列`,`辞書`,`範囲`,`列挙`,`関数`,`型`,`規約`,`算出`の様に、要素や定義内容を内部に持つオブジェクトを複合オブジェクトと呼びます。  
複合オブジェクトの定義方法について、以下説明します。

#### 定義形式のバリエーション

複合オブジェクトは、以下の様な形式で内容を定義することができます。  
ここで<型名>とは、`配列`,`辞書`,`範囲`などの型の名前のことです。  
...
- １文で定義が完了する場合は、ブロック記号`【】`を省略可能です。
- 読点`、`も省略可能です。

以降の各型の説明では、これらのバリエーションの記載を省略します。

#### ブロック内識別語と構文

前記の`<定義内容>`にも共通する定義形式があります。  
定義内の塊を以下の様な構文で表現します。

```
構文: <識別語>(助詞)(読点)<ブロック> | <文>(句点)
```

- <識別語>は、塊を識別するキーワードであり、以下の様なものがあります。  
`要素`,`条件`,`引数`,`設定`,`取得`,`初期化`,`型の要素`,`準拠する規約`,`条項`,`本体`,`処理`
- 助詞：「は」「が」または省略
- 読点、句点： 省略可
- ブロック： ブロック記号`【】`で囲われた複数文。

*構文の例:*

```
１から反復【条件が、【100より小さい。かつ、奇数である】間、1を足す】。(`条件`が識別語。)
１から反復【条件は、100より小さい間、1を足す】。(`条件`が識別語。条件のブロック記号は省略)
```

#### ブロック内識別語の省略

`関数`,`算出`,`型`における識別語「**本体**」と、`反復`における識別語「**処理**」は、それぞれの型でメインの定義となるため、**省略することができます**。  

その場合、ブロック記号も省略し、型定義のブロック内に定義されているものとして扱われます。  
ただし、この場合、型定義ブロックのブロック記号は省略できません。

*識別語省略の例：*

```
加算は、関数【入力がaとbで、aにbを足す】。(`aにbを足す`が「本体」)
型であって、【
    初期化は、【行列は、配列【】】。
...
】
(「入れる」と「出す」の定義文が「本体」)
合計は０。1から９まで反復【入力は数字。合計は、合計に数字を足したもの】。合計。
(「合計は、合計に数字を足したもの」が「処理」)
```

なお、`関数`および`算出`については[関数](#関数function)を、`型`および`規約`については[型および規約](#型typeと規約protocol)を、`反復`については、[反復文](#反復文loopwhile-for-statement)を、
それぞれ参照してください。

### 複合オブジェクトの代入・設定

複合オブジェクト（`配列`、`辞書`、`型`、生成された`インスタンス`など）は、要素に対して値を設定することができます。  
複合オブジェクトがリテラルである場合は値を**変更できない**ため、要素への変更は変更後の新しいリテラルを生成して返します。  
一方、識別子に定義された複合オブジェクトは、[`代入文`](#代入文assignment-statement---オブジェクトの要素への代入設定)または[述語`代入`](#代入assign)により、その識別子の値を変更できます。  

また、`型`および`インスタンス`は、[述語`設定`](#設定set)による要素の設定に対応しており、必要に応じて設定方法を独自に定義できます。  
なお、リテラルに対する設定は、**通常の要素への代入として扱われ**、変更後のリテラルを返します。

*複合オブジェクトへの代入・設定の例:*

```
(1) 配列【1,2,3】の1番目に0を代入。(配列【1,0,3】)
(2) 配列【1,2,3】の1番目は、0。(配列【1,0,3】)
(3) 配列【1,2,3】の1番目に0を設定。(配列【1,0,3】)
```
(1)〜(3)は全て同じ結果になります。  
(2)は、`代入文`
...
数値に続く文字列は、読みやすさを高めるための**助数詞**や**単位**として記述できます。  
これらの文字列は、解析時に無視されるため、 **<数値><文字列>** は、単に **<数値>** と同じ意味になります。
...
ただし、`個`は助数詞として扱わず、特別な助詞として扱います。  
そのため、「3個」は１つの句として述語に渡され、解析時に無視されません。  

例えば、3 個の要素`3`をもつ配列を作る場合は、次の様に記述します。([配列](#配列の要素定義初期化)参照)
...
- `ある`(Be)/`ない`(Not)  
  直前の式を肯定/否定し、結果を真偽値で返します。

```
形式：<式>である(<式>の結果を真偽値で返す)
形式：<式>でない(<式>の否定を真偽値で返す)
```

1. 式(または演算結果)が、数値、文字列等のリテラル値である場合、真を返します。 
1. 式(または演算結果)が、無である場合、偽を返します。  
1. 式がない(空の)場合、エラーを返します。
...
文字列は「」。文字列が空である。(真)
1は2より小さくない。(偽)　
１が２より大きい場合【10】である場合【20】。(エラー：「ある」で評価すべき値が空だった。)
```
...
[論理演算](#論理演算)にある述語`ある`と`ない`により、値があるか無いかを判定する以下の様な記述が可能です。
...
値が複数の場合、「と、」または記号「、」(または「,」(コンマ))で区切ります。  
各要素の型は問いません。
...
以下の述語 `挿入`(insert)/`代入`(assign)/`削除`(remove) では、配列に操作を行う際に、要素取得と同様の**位置指定**をします。

```
形式１：<配列>の<位置>に<要素>を挿入/代入する。
形式２：<要素>を<配列>の<位置>に挿入/代入する。
形式３：<配列>の<位置>を削除する。
```

位置は、0から始まる**数値**、または**数値を示す識別子**です。  
または、位置の代わりに識別子`最初`/`先頭`/`最後`/`末尾`で、配列の位置を指定することもできます。  
操作結果は、基本的にスタックに返ります。  
`代入`では、<配列>として識別子を指定した場合、スタックではなく、その識別子に結果が代入されます。

_`挿入`/`削除`の例：_

```
(1) 配列【1, 2, 3】の先頭に、0を挿入する。(配列【０、１、２、３】)
(2) 配列【1, 2, 3】の1に、0を挿入したものの1番目。(0)
(3) 配列【1, 2, 3】の1番目を削除する。(配列【１、３】)
(4) 配列【1, 2, 3】の末尾を削除する。(配列【１、２】)
```

_`代入`の例：_

```
(1) 配列【１、２、３】の１に５を代入する。(配列【１、５、３】)
(2) 甲は、配列【１、２、３】。乙は０。甲の乙に５を代入する。(甲は配列【５、２、３】)  
(3) 配列【１、２、３】の3番目に５を代入する。(エラー：指定位置が、配列内にありません。(位置：3))
```
２番目の例では、乙が０であるため、指定位置`０`の`１`は`５`に代わり、甲が識別子であるため**結果は甲に代入**されます。

注１：`挿入`/`削除`では、**指定位置が配列の範囲外である場合(操作対象がない場合)**、エラーとはせず**元の配列をそのまま返します**。  
注２：`代入`では、**指定位置が配列の範囲外である**場合、エラーになります。(以下の例参照)  
注３：配列の`削除`について、上記以外は、述語[削除](#削除remove)を参照してください。  
注４：後述の[代入文](#代入文assignment-statement---要素への代入設定)を使うことで、同様の結果が得られます。
...
```
形式１：<配列１>(と)<配列２>(と<配列３>)を足す。
形式２：<配列>を逆順にする。(配列の要素を逆順にした配列を返す)
形式３：<配列>の数。(配列の要素数を数値で返す)
形式４：<配列>が空。(配列が空(から)の場合、真を返す)
形式５：<配列>が<値>を含む。(配列の要素にオブジェクトが含まれる場合、真を返す)
形式６：<配列>が<範囲>を含む。(配列に範囲が含まれる要素(数値)がある場合、真を返す)
```
...
漢数字表は、辞書【要素が、「一」が1と、「二」が2と、「三」が3】。漢数字表の「三」。(3)
漢数字表の「四」。(無)
辞書【真が０、偽が１】の偽。(1)
```
...
```
変換表は、辞書【「一」が１、「二」が２、「三」が３、「四」が４】。変換表の「四」。(4)
変換表に、「四」が無を追加して代入する。変換表の「四」。(無)
```
...
注：キーが存在しない場合、エラーとはせず元の辞書をそのまま返します。
...
```
形式１：<辞書>の<キー>に<値>を代入する
形式２：<辞書>の<識別子>に<値>を代入する
形式３：<値>を<辞書>の<キー>に代入する
形式４：<値>を<辞書>の<識別子>に代入する
```

`代入`により、指定したキーに<値>が代入された辞書オブジェクトが返ります。  
<辞書>として<識別子>を指定した場合は、上記辞書オブジェクトが<識別子>に上書きされます。

注１：キーがハッシュ化可能な型（`数値`、`文字列`、`真偽値`）以外の場合、「エラー：キーが、ハッシュ化可能な型ではありません。型：x」になります。  
...
```
(1) 辞書【1が「一」、2が「二」、3が「三」】の2に「弍」を代入する。(辞書【1が「一」、2が「弍」、3が「三」】が返る)
(2) 辞書【1が「一」、2が「二」、3が「三」】の4に「四」を代入する。(辞書【1が「一」、2が「弍」、3が「三」、4が「四」】が返る)
(3) 甲は、辞書【「い」が「一」、「ろ」が「二」、「は」が「三」】。甲の「は」に「参」を代入する。(甲は辞書【「い」が「一」、「ろ」が「二」、「は」が「参」】)
(4) 甲は、辞書【真が1、偽が0】。甲の偽に2を代入する。(甲は辞書【真が1、偽が2】)
(5) 甲は、辞書【1が「一」、2が「二」、3が「三」】。乙は２。関数【「弍」を甲の乙に代入する。】を実行する。(辞書【1が「一」、2が「弍」、3が「三」】)
```
...
列挙は、**列挙子(enumerator)** と、それに紐づけられた**値（任意のオブジェクト）** の集合です。  

また、**列挙子**は、列挙の要素を識別する**識別子**です。  

#### 定義

列挙リテラルの定義は、以下の様になります。
...
注：**形式１**は、[定義文](#定義文define-statement)と同じ形式です。

#### 要素の値

**要素の値**は、**形式１**の様に指定します。  
**形式２**の様に、要素の値を指定しない場合、値は、**0**から始まる**連続した数値**となります。 

さらに、**値**に**数値**を指定した場合は、値の指定を省略した続く要素の値は、1を足した**連続した数値**となります。  
...
aとbは、値を指定していないので、それらの値は、**0から始まる連続した数値**、すなわち、「0」と「1」になります。  
また、dも、値を指定していませんが、cに値「10」が指定されているため、dの値は、**1を足した**「11」になります。

#### 要素のアクセス

列挙オブジェクトが持つ要素は、以下のいずれかの形式で参照できます。
...
要素や属性のアクセスには、通常、**形式１**の様に属格の助詞`の`を使いますが、  
列挙では、**形式２**と**形式３**の様に記号`・`(中黒、中点)を使って要素をアクセスすることができます。

**形式３**は、**形式２**の**省略形**で、列挙オブジェクトが辞書に登録されている場合、つまり、定義文で定義されている場合に使えます。

いずれも指定した**列挙子**を持つ**要素**のオブジェクトが返ります。

さらに、上記の様に得られた要素オブジェクトについて、その属性や値を以下の様にアクセスすることができます。
...
(4) ・火の列挙子。(「火」)
(5) ・火の値。（「火曜」）
(6) 木曜日は、・木。木曜日が、曜日の木に等しい。(真)
(7) 列挙【月、火、水、木、金、土、日】の日の型。(「」(空文字を返す))
```

(7)では、「曜日」と異なり、列挙に識別子が紐づけられていないため型名が「」になります。

#### 要素の比較

２つの<要素>が等しいか否かは、述語`等しい`を使います。

```
形式：<要素１>(が)<要素２>に等しい。(２つの要素が等しい場合に真を返す)
```

２つの要素の`型`、`列挙子`、`値`がすべて等しい場合に、真になります。

#### 値の代入(Assign)

述語`代入`(assign)によって、値を代入した列挙**要素オブジェクト**を得ることができます。
...
注： 列挙で定義している要素の値を更新することはできません。**列挙の要素(列挙子と値)は不変**です。
...
述語`生成`(create)を用いて、**要素の値**から**要素オブジェクト**を得ることができます。

`生成`では、**列挙オブジェクト**から、**要素オブジェクト**を生成します。  
**要素オブジェクト**から、値を代入したオブジェクトを得る`代入`とは異なります。
...
**形式２**と**形式３**では、指定した識別子名の識別子に生成した要素オブジェクトを代入します。 
...
「〜以上〜以下」のような**数値の範囲**を表します。

```
形式：範囲【<範囲式>】
```

<範囲式>は、**数値返す式**と[境界を表す助詞](#特殊な助詞) `から`, `まで`, `以上`, `以下`, `未満` で構成されます。  

また、組み合わせにより、下限、上限、上下限の３通りを表現できます。  

`から`と`以上`は **「下限」** を表し、`まで`と`以下`と`未満`は **「上限」** を表します。  
上下限は、下限と上限を組み合わせます。

上下限を表す場合、**下限→上限**の順で記述します。逆順の**上限→下限**の場合はエラーになります。  
また、上限どうしや、下限どうしも組み合わせることはできません。

注：境界を表す助詞 `から`, `まで`は、内部的に以下の様に変換されます。
- `から` → `以上`
- `まで` → `以下`
...
さらに、<範囲式>には、数値を返す[単文](#単文)を使用することができます。

*単文使用の例：*

```
範囲【1に1を足すから、100を10で割るまで】(範囲【2以上、10以下】)
```

ただし、単文内で **境界を表す助詞** を使用することは、基本的にできません。  

*例：*

```
範囲【10から1を引くから10に1を足すまで】
(エラー： 範囲式の単文には、引数に境界助詞を持つ述語「引く」を使用できません。
        上下限を含む範囲式の場合、読点「、」で上下を区切ってください。 )
```

例では、下限助詞`から`が単文中で使用されており、境界位置が判別できないため、上記**エラー**になります。  

この様な場合、エラーメッセージの二行目にある様に、**上下限の間を読点「、」で区切る**ことでエラーを回避可能です。  

```
範囲【10から1を引くから、10に1を足すまで】 (範囲【9以上、11以下】)
```

読点で区切ることにより、境界位置が明示され、助詞によらずに解析可能になります。
...
境界を表す助詞`以上`、`以下`、`未満`をよび記号`〜`を用いることにより、予約語`範囲`を使わずに**範囲リテラル**を記述できます。  
`から`と`まで`は、**省略記法として用いることはできません**。
...
予約語`範囲`では、**単文**の使用が可能ですが、**省略記法では使えません**。

```
(1) 範囲【１と２を足す以下】 (範囲【3以下】)
(2) １と２を足す以下 (エラー：「上下限式は、「数値」または「識別子」のみです。」)
```

- 形式４「**<式>〜**」は、形式１「**<式>以上**」と同じです。  
- 形式５「**〜<式>**」は、形式１「**<式>以下**」と同じです。  

`〜`で、形式３「**<式>未満**」を表す場合、以下の様に記述します。

```
形式６：〜<式>未満
```

さらに、組み合わせにより、上下限範囲を記述することもできます。
...
また、ブロックや、定義文を、定義内容とすることはできません。定義内容は、値を返す述語を持つ単文、および複文です。  
そして、同じ<識別子>に定義した場合は、再定義(上書き)になります。
...
例のとおり、「甲を表示し」が**１文**であるため、**ブロック`【】`の省略が可能**です。   
また、連用形で句点を付けずに文を続けることができます。(終止形で句点を付けることも可)  
...
```
甲が、1 の場合【「い」を表示し】、2 の場合【「ろ」を表示し】、それ以外【「ん」を表示する】。
```
...
注：「１の場合」「２の場合」は、行末が、それぞれ１文であるため、ブロック記号`【】`を省略しています。
...
```
形式１：反復【(条件が、<条件式>間、)(処理が)<実行処理>】。
形式２：<数値>から<数値>まで（<数値>ずつ）反復【入力が<識別子(カウント値)>であり、(処理が)<実行処理>】。
形式３：<範囲>を反復【入力が<識別子(カウント値)>であり、(処理が)<実行処理>】。
形式４：<配列>を反復【入力が<識別子(値)>であり、(処理が)<実行処理>】。
形式５：<辞書>を反復【入力が<識別子(キー)>と<識別子(値)>であり、(処理が)<実行処理>】。
```

注：反復文の入力を示す「入力が」(または「入力は」)部分の終端(文字)には、句点「。」または「で」が必要です。  
「で、」「であり、」も可能ですが、読点「、」単独では終端になりません。  
...
接続詞`かつ`、`または`は、２つ以上の論理式を接続し、論理演算を行います。  

論理式は左から順に評価されますが、`かつ`は`または`よりも 優先して評価されます。
また、ブロック`【`と`】`で囲うことによって、評価のまとまりや順序を明示的に指定できます。
...
`かつ`と`または`が、混在する場合、`かつ`は`または`よりも**優先して評価**されます。
...
ブロック記号を用いることで、評価の優先度を明示的に指定できます。

```
1が2に等しい、かつ、【2が2に等しい、または、3が4に等しい】。(真)
1が2に等しい、かつ、【2が2に等しい、または、3が3に等しい】。(偽)
```

ブロック【】で囲まれた論理式は、その内部が優先して評価されます。

#### ブロックの使用に関する注意

接続詞`かつ`、`または`は、**右側の論理式のみ**をブロックとして受け取ります。  
そのため、**次のような記述はできません**。

```
【1が1に等しい、または、2が2に等しい】、かつ、3が4に等しい。
```

この様な評価をしたい場合は、条件を**接続詞の右側にブロックとして記述**してください。
...
論理式は、句点`。`で区切って記述することもできます。  
この場合、前の文の評価結果が、次の文の条件として用いられます。
...
上記は、次のように評価されます。  
1. `1が1に等しい、または、2が2に等しい`を評価
1. その結果に対して`かつ、3が4に等しい`を適用

この書き方は、**ブロックを用いた場合とは評価の意味が異なる**ことに注意してください。
...
`場合文`と異なり、処理(ブロック)ではなく値(式)であり、`【】`は不要です。また、<値２>の省略は不可です。  
<条件>の後の`か`は、<条件>が述語で終わる単文(条件式)の場合、必要になります。(`か`は他に作用をしません。)
...
```
形式：関数【(入力が、<入力定義>。)(出力が、<出力定義>。)(本体が、)<実行処理>】
```

#### 関数の入力定義

<入力定義>は、基本的に識別子を助詞`と`を使い、入力する識別子を必要個並べます。  
(＊入力の識別子は、既存プログラミング言語では、**仮引数(Parameter)** と呼ばれます。)  
...
複数の引数を入力する場合、渡された**引数の順**に識別子に代入(入力)されます。  
たとえば、関数「加算」の入力が識別子`a`と`b`であった場合、`１と２を加算する。`の引数`1と`、`2を`は、`a`に 1、`b`に２がそれぞれ代入されます。(注：入力の際、助詞`と`と`を`は省かれます。)

引数の数が、入力として指定した識別子の数に満たない場合、エラーとなります。

#### 既定値を持つ入力（Default Parameter Values)

`関数`の入力識別子は、以下の形式で **既定値(default value)** を持つことができます。

```
形式：入力が、<識別子１>は<既定値１>(と、<識別子２>は<既定値２>と、…)。
```

**既定値**を指定した識別子は、関数に渡す引数を**省略することが可能**です。

ただし、識別子を`甲「と…」`の様に**可変長**に指定した場合[(入力チェックを参照)](#入力チェック)、**既定値**を持つことはできません。　　

また、既定値を指定した識別子と、指定していない識別子が**混在する場合**、既定値を持たない識別子を**先に定義**してください。  
(引数は、先頭から割り当てるため、後ろの識別子に**正しく値を割り当てることができなくなります**。)

注：識別子に<型>を指定した場合、既定値の型と指定<型>が一致しないと、エラーになります。
...
例では、入力定義の`x`は既定値が定義されておらず、`y`と`z`は既定値を定義しています。 
...
例では、３つの識別子が定義されています。識別子`呼びかけ`は、**既定値**「こんにちは」を持ちます。  
`相手１`は、可変長識別子であり、既定値を指定することはできません。
...
**値を返し実行処理を終了**します。(`返す`に続く文、式等は無効です。)  
返す<値>は必須です。

`返す`を使わずに、暗黙的に返り値をオブジェクトとして**スタックに格納**し、その後、別の処理を行うことも可能です。

また、`返る`(return)は、**値を返さずに実行処理を終了**する場合に使います。
...
実行時に **入力(引数の型と格)** をチェックすることが可能です。チェックするには、入力の識別子にかぎ括弧`「`と`」`を付けて、以下の様に記述します。
...
- …(三点リーダ)  
  ...
  すなわち、指定した連続する型と格を持つ引数(可変長)の値を配列にし、識別子に渡します。  
  一致する引数が無い場合は、エラーではなく、識別子に空の配列が渡されます。
...
多重定義にシグネチャが一致する関数が見つからない場合は、`エラー：入力形式が一致する関数が見つかりません。`が出力されます。
...
<出力定義>として、**型名**を助詞`と`を使い、**出力する順番に必要個並べる**ことにより、実行時に関数の出力(返り値の型)をチェックします。  
(＊出力の型は、既存プログラミング言語では、**返り値の型(Return Type)** と呼ばれます。)  

出力のチェックが不要な場合は、**出力定義は不要**です。

関数の出力定義をすることにより、出力の仕様を明示することができます。
...
注：「出力が」(または「出力は」)部分の終端(文字)には、句点「。」または「で」が必要です。「で、」「であり、」も可能ですが、読点「、」単独では終端になりません。

**形式１**では、関数の出力(返り値またはスタック)の**値の型**を順にチェックします。  
値の型が指定の型名と一致しない場合は、エラーとなります。  
また、値の数が、指定に満たない場合も、エラーになります。

**形式２**では、関数の出力が**無いこと**をチェックします。  
関数に出力がある場合は、エラーになります。  

形式１で指定する型は、[組み込みの型名](#値literalと型type)、または、[`型`](#型type)、[`規約`](#規約protocol)、および[`列挙`](#列挙enumeration)で定義した識別子になります。

なお、以下の様に、型名の後に **「か無」** や **「かエラー」** を続けることで、複合的な出力型（いずれかの型を取りうる出力）を定義できます。
...
**「<型>か無」** は、Swiftの **Optional<型>** に相当し、出力が値を持たない（無）場合も正しい出力と判断されます。  
また、**「<型>かエラー」** は、Swiftの **Result<型, エラー>** に相当し、出力が指定の型、または、エラー型のいずれかである場合に正しいと判断されます。

*出力チェックの例：*
```
(1) 関数【出力は「数値」。1】を実行。(1)
(2) 甲は算出【出力は「文字列」。「い」を返す】。甲。(い)
(3) 関数【出力は「数値」と「文字列」。1、「い」を返す】を実行。(1 い)
(4) 関数【出力は「」】を実行。()
(5) 関数【出力は「数値か無」。無】を実行。(無)
(6) 関数【出力は「数値かエラー」。１を０で割る】を実行。(エラー：1を0で割ることはできない。)
```

(6)の例では、出力が**エラー型**なので、型名が一致し結果エラーを出力します。

*出力チェックのエラー例：*
```
(7) 関数【出力は「数値」。「い」】を実行。(エラー：出力の型が指定と異なります。実際の型：文字列）
(8) 関数【出力は「数値」。１を０で割る】を実行。(エラー：出力の型が指定と異なります。実際の型：エラー）
(9) 関数【出力は「数値」と「文字列」。1】を実行。(エラー：出力の数が足りません。必要数：2)
(10) 関数【出力は「」。1】を実行。(エラー：期待しない出力(型：数値)がありました。)
```

(8)の例では、(6)と異なり、**エラー型**をチェックしていないため、**「出力の型が指定と異なります」** というエラーになります。  
(10)では、出力なしのチェックで、出力`1`を検出しエラーになっています。
...
(注：この場合、出力はシグネチャに**含まれません**。) 

また、`さらに`を付けずに定義した場合は、**上書きされ**最新の定義が有効になります。
...
関数はクロージャに対応しており、関数外で定義された識別子をキャプチャすることができます。([算出](#算出computation)は引数にできないため、クロージャに未対応)
...
２つ目は、`型`の値要素として定義し、定義した値の識別子をアクセス(取得と設定)したときの実行処理を表現することです。  
 (既存のプログラミング言語ではアクセサ、あるいはコンピューテッドプロパティと呼ばれ、取得は getter、設定は setter と呼ばれています。)

１つ目については、関数の定義と同じ形式になります。

```
形式１：算出【(入力が、<入力定義>。)(出力が、<出力定義>。)(本体が、)<実行処理>】
```
...
形式２：
  算出【
    設定が、【(入力が、<入力定義>。)(出力が、<出力定義>。)(本体が、)<実行処理>】
    取得が、【(入力が、<入力定義>。)(出力が、<出力定義>。)(本体が、)<実行処理>】
  】。
```

形式２では、「設定」と「取得」の定義を１つの`算出`で行います。(設定、取得の順番は替えられません。「取得」の定義を省略し、「設定」だけにすることは可能です。)

`算出`は、`関数`と違い、**[関数の実行(呼び出し)](#関数の実行function-call)を必要としません。** すなわち、`算出`には、関数呼び出しのための述語`実行`あるいは述語`する`が不要で、実行はその評価時に行われます。
...
形式１は、形式２の「**設定**」定義が無い形式で、動作に差はありません。  
すなわち、形式２の「**取得**」は、形式１と同等です。

なお、動詞や形容詞の識別子をより日本語らしい表現で定義するための構文「**算出定義文**」を用いることができます。(後述の[算出定義文](#算出定義文-computation-definition-statement)を参照)
...
定義した**要素**は、後述する型から生成されたオブジェクト(**インスタンス**)が持つ辞書に登録されます。

#### インスタンスの生成(New Operator)

`型`は、雛形であり、以下の述語`生成`によって生成されたオブジェクトが、実体(**インスタンス**)となります。  
...
】
カウンターは、計数から生成。
(1) カウンターを増やす。カウンターの現在値。(1)
(2) カウンターを5個増やす。カウンターの現在値。(6)
(3) カウンターをリセット。カウンターの現在値。(0)
```
...
そして、`カウンターの現在値`を実行することにより、算出が呼ばれ、`カウント値`が返ります。
...
型からインスタンスを生成する際に行う「初期化処理」(initializer)を`型`に定義することができます。  
初期化処理には、インスタンス生成時の引数(入力)を指定することが可能です。  
同じ型でも異なる引数を与えることで、異なる値や振る舞いを持つインスタンスを生成できます。

引数の形式は、[関数の入力定義](#関数の入力定義)と同じであり、同様に[入力チェック](#入力チェック)も適用されます。

```
形式：初期化が、【<関数定義>】
```

<関数定義>の入力形式は、[関数](#関数function)や[算出](#算出computation)と同じです。  

また、初期化処理で、渡された**引数**、および処理中に定義した**識別子**は、インスタンスの辞書に登録されるため、**インスタンス要素**として使用可能です。

#### 初期化の出力チェック
初期化処理にも出力定義を記述することで、[出力チェック](#出力チェック関数の出力定義)を行うことができます。  
ただし、初期化処理の出力形式は`関数`や`算出`とは異なり、以下のいずれかになります。
```
形式１：出力が「自身」
形式２：出力が「自身か無」
形式３：出力が「自身かエラー」
...
初期化処理では、通常、値を返す(出力する)ことはありません。  
しかし、初期化が成功した場合には、生成されたインスタンス自身が暗黙的に出力されます。この出力を、出力定義では「**自身**」と表現しています。

形式１の場合、正常にインスタンスが生成された場合にチェックは成功します。

一方、初期化が失敗した場合には、`無`または`エラー`を出力することができます。  
そのため、許容する出力形式として「自身か無」「自身かエラー」を指定できます。  

(Swiftでは、初期化失敗時にnilを返す初期化(イニシャライザ)を、**失敗可能イニシャライザ(failable initializer)** と呼んでいます。ふぉーすでは、**nil**の代わりに`無`を使います。)　　

実行時に、定義された出力形式と異なる出力があった場合は、エラーとなります。
...
##### 自身(self)の初期化(初期化処理の呼び出し)

また、`初期化`の多重定義において、インスタンス自身を表す`自身`(self)を使って、１つの定義から他の初期化処理を呼び出すことができます。  

`自身`は、インスタンス内の辞書に登録されているため、他の要素からも同様にアクセス可能です。
...
また、通常の関数定義と同様に、入出力の指定も可能です。(算出の詳細については、[算出(Computation)](#算出computation)を参照)
...
**形式１**では、ブロック内に複数の要素を定義できます。  
**形式２**では、単独の要素を複数定義できます。

２つの形式は混在も可能ですが、**形式１→形式２の順に定義する必要**があり、形式１の定義は、**型の定義内で１つのみ**です。  
また、形式２は、`本体`の定義内に記述することはできません。

要素へのアクセス方法は、インスタンスではなく**型の識別子(型名)を指定**する以外は、インスタンスと同じです。([インスタンス要素のアクセス](#インスタンス要素のアクセス)参照)

`型の要素`と`インスタンス要素`の違いは、以下の通りです。

|種類|定義|アクセス方法|`利用可能`の指定
|--|--|--|--|
|**型の要素** | 型の要素が、【<識別子>は<定義>…】<br>型の<識別子>は<定義>…|<型>の<要素(識別子名)>|必要
|**インスタンス要素**|　本体が、【<識別子>は<定義>…】|<インスタンス>の<要素(識別子名)>|不要
...
】。
挨拶の本文を表示。(「こんにちは、田中さん。」)
挨拶の宛名を「佐藤」に設定した挨拶の本文を表示。(「こんにちは、佐藤さん。」)
```

型「挨拶」の要素「宛名」を変更し、「本文」を表示しています。
...
注：規約では、型の定義における「型の要素が、【〜】」を使った型の要素の定義はできません。

さらに、`関数`、`算出`(`設定`および`取得`)、`初期化`では、必要に応じて、その入出力形式を規定することができます。

```
形式３：(型の)<識別子>は、関数【入力が<入力形式>。出力が<出力形式>】
形式４：(型の)<識別子>は、算出【入力が<入力形式>。出力が<出力形式>】
形式５：(型の)<識別子>は、算出【
                          設定は、【入力が<入力形式>。出力が<出力形式>】
                          取得は、【入力が<入力形式>。出力が<出力形式>】
        】
形式６：初期化は、【入力が<入力形式>。出力が<出力形式>】
```

(注：形式５で、「取得は、」の定義は、省略可。)

ここで、入力形式とは、関数の[入力チェック](#入力チェック)で説明している以下の形式です。

```
形式：<識別子>「<型><格>…」
```

これを「と」で繋いだ複数の識別子を指定できます。

同様に、出力形式を指定することもできます。  
出力形式とは、[関数の出力定義](#出力チェック関数の出力定義)で説明している以下の形式です。
...
これにより、`関数`または`算出`の出力の数と型を規定することができます。

`初期化`も出力の指定ができますが、形式は、以下のとおりです。
```
形式１：出力が「自身」
形式２：出力が「自身か無」
形式３：出力が「自身かエラー」
```
詳細の説明は、[初期化の出力チェック](#初期化の出力チェック)を参照してください。

また、`関数`と`算出`で、入出力形式の規定が不要である場合は、形式１、２の様にそれぞれの<型名>だけ記述します。  
例えば、識別子「甲」が関数であることだけをチェックし、入出力のチェックが不要の場合、条項は、
```
甲は、「関数」
```
と書きます。

また、**入力定義**を使って１つの識別子に複数の規定をすることができます。すなわち、準拠する型に、**多重定義**を求めることができます。  
規約で多重定義の規定をする場合、「さらに」は特に意味を持たないため、省略することも可能です。  

#### 規約の準拠方法

`型`を、規約に準拠させるためには、最初に[`準拠する規約`の定義](#準拠する規約)をします。  
さらに、規約の条項の定義に沿った型を持った<識別子>を定義します。  
必要に応じて、`関数`と`算出`では、指定した入出力形式に従った`関数`、`算出`を定義します。  

準拠する規約が、さらに規約に準拠している場合、`型`は、すべての規約に準拠する(条項を実装する)必要があります。

規約に準拠した定義が見つからない、または、入力形式が異なる場合は、エラーメッセージを出力します。

なお、**規約の条項で定義した識別子は、準拠した型で自動的に`利用可能`となる**ため、[`利用可能`の定義](#インスタンス要素のアクセス)は不要です。
...
表記は、関数であって、【入力が通貨国「国名」で、出力が「通貨表記か無」。
    通貨国が、
...
・スペインで表記した記号を表示する。(€)
・米国で表記したコードを表示する。(USD)
・英国で表記したコードを表示する。(『コード』(識別子)が定義されていません。)
```

例では、関数「表記」は、入力の国名に応じて、規約「通貨表記」に準拠した型(「ユーロ」や「米ドル」)からインスタンスを生成します。  
これにアクセすることにより、通貨の「記号」と「コード」を表示します。  

インスタンスの生成が定義されていない国名「英国」が入力の場合、関数「表記」は`無`を返し、未定義の「コード」を参照するため、エラーが出力されます。(「無」はスタックに出力される。)

注１：規約『通貨表記』の二重かぎ括弧は規約を目立たせる目的で、無くても構いません。また、「表記した」は「表記し、」でも同じです。  
注２：関数「表記」の出力定義で、当該関数が「通貨表記か無」を返すことを明示しています。これにより、使用する側は、入力の国名に対応した「通貨表記」の「記号」および「コード」にアクセス可能であることを知ることができます。  
注３：規約の条項「記号」と「コード」は、準拠した型「ユーロ」「米ドル」のインスタンスで`利用可能`(外部アクセス可能)です。
...
この実装定義により、`型`で条項の識別子（関数や算出など）を定義しなかった場合の、既定の振る舞いを定義することができます。
...
オブジェクトに値を代入します。  
対象となるオブジェクトは、識別子、配列の要素、辞書の要素、列挙の要素です。
...
計算に使える述語は以下の通りです。
- `足す`、`掛ける`、`割る`、`引く`、`負数`、`追加`、`挿入`、`削除`、`逆順`(注）

識別子は、**文頭にある**ものが対象で、**定義済みである必要**があります。  
...
形式６：<配列>の<数値>に<値>を代入
形式７：<値>を<配列>の<数値>に代入
形式８：<配列>の<識別子>に<値>を代入
形式９：<値>を<配列>の<識別子>に代入
```
...
```
形式１０：<辞書>の<キー>に<値>を代入する
形式１２：<辞書>の<識別子>に<値>を代入する
形式１３：<値>を<辞書>の<キー>に代入する
形式１４：<値>を<辞書>の<識別子>に代入する
```
...
列挙の要素に値を代入したオブジェクトを得ることができます。([列挙の要素に代入](#値の代入assign)を参照)
...
注：いずれも、列挙の要素に<値>を代入した<要素オブジェクト>を返します。
...
```
(1) 甲は配列【１、２、３】。乙は１。関数【甲は１。外部「甲」の乙に０を代入】を実行。甲。(配列【1,0,3】)
(2) 甲は配列【１、２、３】。乙は２。関数【乙は１。外部「甲」の外部「乙」に０を代入】を実行。甲。(配列【1,2,0】)
```

(1)では、関数で外部の`甲`と`乙`にアクセスし、配列`甲`に代入します。  
(2)では、関数で配列`甲`の位置`乙`(外部)に 0 を代入します。
...
```
形式１：<値>(を)<オブジェクト>の<識別子>に設定する。
形式２：<オブジェクト>の<識別子>に<値>を設定する。
形式３：<オブジェクト>の<識別子>を<値>に設定する。
```

注１：形式２と形式３では、格助詞`に`と`を`が逆になっています。  
注２：[`代入文`による設定](#代入文assignment-statement---要素への代入設定)も可能です。  
注３：オブジェクトが`配列`や`辞書`などのリテラルである場合、`設定`は、[`代入`](#代入assign)として動作します。
...
```
形式１：<配列>の<数値(位置)>を削除する。
形式２：<配列>の先頭/末尾/全て/残りを削除する。
```

`配列`の**位置を指定**した削除については、[配列の位置指定と操作](#配列の位置指定と操作)を参照してください。  

「全て」を指定した場合、**空の配列**が返ります。  
また「残り」を指定した場合、配列の先頭以外が返ります。

*例：*

```
(1) 配列【１、２、３】の全てを削除。(配列【】)
(2) 配列【１、２、３】の全てを削除。(配列【２、３】)
```
...
注３：キーが存在しない場合、エラーとはせず元の辞書をそのまま返します。
...
ディレクトリパスを表示する。(エラー：『ディレクトリパス』(識別子)が定義されていません。)
```

さらに、以下の様にオブジェクト内の要素(値)を検索し、それを削除することも可能です。  
「最初」、「全て」を指定することにより、検出した最初または全ての要素を削除します。

```
形式６：<オブジェクト>から(最初の/全ての)<値>を削除する。
```
注１：「最初」は「先頭」でも良い。  
注２：「最初」や「全て」の指定を省略した場合は、最初の１つを削除します。


*要素の検索削除の例：*

```
(1) 配列【1, 2, 3】から3を削除したものの数。(2)
(2) 配列【1, 2, 3, 2】から２を削除したものの1番目。(3)
(3) 配列【1, 2, 3, 2】から、最初の２を削除したものの1番目。(3)
(4) 配列【1, 2, 3, 2】から、全ての２を削除したものの数。(2)
(5) 辞書【「その一」が１、「その二」が２、「その三」が３】から２を削除したものの「その二」。(無)
(6) 辞書【「a」が１、「b」が２、「c」が３、「d」が２】から全ての２を削除したものの数。(2)
```
...
```
形式１：(<識別子１>(と<識別子２>…)に)(n個)得る。
形式２：(<識別子１>(と<識別子２>…)に)中身を(n個)得る。
形式３：(<識別子１>(と<識別子２>…)に)数値を(n個)得る。
```



1. <識別子>を指定すると、<識別子>にスタックから値を取得し、代入する。
1. 「中身」を指定すると、取得値から中身(格を除いた値)を取り出す。
1. 「数値」を指定すると、取得値を数値に変換する。(変換できない場合は**無**)
1. 個数を指定すると、指定数分スタックから値を取得し、**配列**に出力する。  
１つの識別子を指定した場合、**上記配列**を、識別子に代入する。
1. 複数の識別子を指定した場合、識別子ごとに値を代入する。(同時に個数の指定がある場合は、**エラー**)

※：指定個数、または識別数分、スタック値を取得できない場合は**エラー**

_`得る`の例：_

```
1を積む。甲は得たもの。甲。(1。甲に１が代入され、スタックは空になる。)
2を積む。甲に得る。甲。(2。甲に２が代入され、スタックは空になる。)
13 14 15を積む。甲に３個得る。甲。(配列【13, 14, 15】)
16を積む。甲は3個得たもの。甲。(無)
17と18を積む。捨てる。甲に「数値」を得る。甲。(17)
...
形式２：(<識別子１>(と<識別子２>…)に)中身を(n個)写す。
形式３：(<識別子１>(と<識別子２>…)に)数値を(n個)写す。
```

_`写す`の例：_

```
10と11と12を積む。甲に３個写す。甲。(10と 11と 12 配列【10と、11と、12】。)
※ スタックに積んだ値はそのままで、配列にコピーされた値が甲に代入される。
25と26を積む。甲と乙に写す。甲から乙を引く。(25と 26 -1)
```
...
`それ`は、直前の値(オブジェクト)を参照する述語で、スタックから値を取り出して返します。  

```
形式：それ
```

`得る`(pull) と機能は同じですが、日本語として自然な文を作りたい場合に`それ`を用いることができます。  

例えば、スタックから値を取り出して計算する場合、`得る`を使うと  
  - 「得て、1を足す」
  - 「得たものに1を足す」  
...
(1) 得たものが無である。(真)
(2) それが無である。(偽)
(3) それに1を足す。(エラー：「足す」には、２つ以上の数値、文字列、配列の入力が必要です。仕様：(〜と…)〜を足す。)
```

(3)では、`それ`が返す値が無く、`足す`の入力が１つのため、この様なエラーが出力されます。

##### 値の属性を得る`その`

`その`は、直前の値(オブジェクト)の属性を得る述語です。  
スタックから取得した値に続く属性を評価し、その結果を返します。
...
スタックが空の場合、属性の対象となる値が存在しないため、`その`は属性を参照できず、エラーを返します。

_空のスタックの例：_

```
その値。(エラー：「その」が指すオブジェクトがありません。(スタックが空))
```

#### 「その」と「それの」の違い(補足)

`その`は 属性アクセス専用の述語であり、後続には属性名（識別子）が来る必要があります。  
そのため、属性名ではない語（数値・述語など）が続くと、構文エラーとなります。

```
配列【1,2,3】。その数。(3)
配列【1,2,3】。その1番目。(「1」は属性名ではありません。(解析位置: その))
１。その負数。(「負数」は属性名ではありません。(解析位置: その))
```

一方、「`それ`の」は 属性アクセスではなく、対象となる値に対する演算として扱われます。  
このため、`1`や`負数`などの述語を続けても構文上問題ありません。  
...
挨拶は、型であって、【
    型の要素が、【宛名は、「田中」。本文は、算出【「こんにちは、」と宛名と「さん。」を足す】】。
】。
挨拶の宛名を「佐藤」に設定した挨拶の本文。(「こんにちは、佐藤さん。」)
挨拶の宛名を「鈴木」に設定した挨拶の本文。(こんにちは、佐藤さん。た こんにちは、鈴木さん。)
```

上記注で示したとおり、例の最後の文`挨拶の宛名を「鈴木」に設定`は、値を返さないため、`し`(する)が、直前のオブジェクト`「こんにちは、佐藤さん。」`を返したことにより、次の助詞`た`をつなげ`句`を作っています。(実行前にスタックを空にすることで、解消します。)
...
(2) 正しいとは、入力がaで、aが真であること。4が5より大きいは、正しくない。(真)
```
...
`等しい`、`小さい`、`大きい`、`ある`、`ない`、  
`表示`、`改行`、`読む`、`尋ねる`、  
`追加`、`挿入`、`削除`、`繰り返す`、`写像`、`まとめる`、`絞り込む`、`並び替える`、`含む`、`逆順`、  
`返す`、`返る`、`実行`、`する`、`中止`、`継続`、
`入力`、`積む`、`捨てる`、`得る`、`写す`、`代入`、`入れ替える`、`生成`、`設定`、`要素`、  
`もの`、`こと`、`それ`、`その`、`それ以外`、  
`ファイル一覧`、`識別子一覧`、`利用可能`、`初期化`
」

**原語**(再定義されていない元の予約語)を使用する場合は、以下の様に指定します。
...
「座標」型の定義内(有効範囲内)で、元の足し算をするには、`《足す》`を使います。  
(注：`《足し》`の様に「足す」を連用形にすることも可能です。)
...
『引く』は、座標 a から座標 b を引く`算出`として、多重定義されています。算出内の数値の引き算には原語`《引き》`を使います。

(1)では、当定義により、座標 A から座標 B を引いた値が、座標として返り、結果の配列が得られます。  
(2)では、「引く」は拡張定義されていますが、座標どうしの引き算でないため、原語`引く`が実行されます。

## 糖衣構文(Syntax Sugar)

述語による記述は、わかりやすい一方、**冗長**になることがあります。  
記述を簡潔にできる様、以下の様な **記述形式(糖衣構文)** を用意しています。

### 算出定義文 (Computation Definition Statement)

語尾変化する動詞・形容詞の識別子は、通常の定義文では**名詞述語**として扱われるため、日本語の用法と乖離します。  
**算出定義文**を用いることで、**動詞・形容詞として自然な文構造のまま算出を定義できます。**

```
形式：<識別子>とは、(さらに、)【<算出(取得)の定義内容>】(こと。)
```

注１：「さらに、」により多重定義可能  
注２：「とは」の後に予約語`算出`は不要です。  
注３：「こと。」は、省略可能  

この文は、識別子を「算出」として定義する**糖衣構文**であり、以下の定義文形式と等価です：  

```
<識別子>は、(さらに、)算出【<算出(取得)の定義内容>】。
```

*算出定義文の例：*

```
(1) 加えるとは、【入力がxとyで、xとyを足す】こと。5に5を加える。(10)
(2) 正しいとは、入力がxで、xが真であること。4が5より大きいは、正しくない。(真)
```


(1)と(2)は、以下の定義文と**等価**です。

```
(1') 加えるは、算出【入力がxとyで、xとyを足す】。5に5を加える。(10)
(2') 正しいは、算出であって、入力がxで、xが真である。4が5より大きいは、正しくない。(真)
```

#### 算出定義文の警告

**算出定義文**では、「とは」の後に予約語`算出`を書く必要はありません。  
`算出`を書いた場合、それは「**算出の中でさらに算出を返す処理**」として解釈されます。  
これは意図しない**二重構造**になるため、以下の**警告**が表示されます。   
...
注１：通常のブロックと異なり、１文で記述できる場合でも、`【】`は省略できません。  
注２：`引数が、`は省略可能です。
...
配列や辞書に値を代入するためには、述語`代入`(assign)を使い、`arrayの1に、3を代入する`の様に記述し、オブジェクトの要素に値を代入します。([配列への代入](#配列に代入)、[辞書への代入](#辞書に代入)を参照)  
また、型やインスタンスの要素に値を設定するためには、述語`設定`(set)を使い、`インスタンスのメンバに値を設定する`の様に記述し、要素に値を設定します。([設定(Set)参照](#設定set))

これらに対し、以下の様な形式の構文(**代入文**)を用いることにより、これらを代替し、簡易に記述することができます。  
代入文では、`代入`や`設定`といった予約語の使用が不要になります。
...
注：  
外側とローカル辞書の両方に同じ識別子がある場合、代入結果は、ローカル辞書に登録されます。外側の識別子を指定するためには、[`外部`](#関数の呼び元への代入)を使用します。
...
(2)では、配列甲の乙番目(0)に５を代入した結果が、甲に代入されます。甲の「最初」は 5 に変更されています。  
...
】。
(1) 挨拶の本文。(こんにちは、田中さん。)
(2) 挨拶の宛名に、「鈴木」を設定する。挨拶の本文。(こんにちは、鈴木さん。)
...
**属性アクセス**は、列挙の[要素の値](#要素の値)に付与した情報へ直接アクセスするための仕組みです。  
要素を属性名で参照できるため、列挙要素をオブジェクト的に扱うことが可能になります。

```
形式：<要素>の<属性> (要素に紐づく属性値を返す)
```

列挙の要素の値には、任意のオブジェクトを割り当てられます。  
この値として `列挙`、`辞書`、`型`を指定した場合、その内部の名前（列挙子、キー、識別子）を、**属性名**として用いることができます。
...
このように、果物の要素「りんご」「みかん」に属性となる`値段`、`色` を持つ列挙を割り当てておくことで、  
**<要素>の<属性>** という簡潔な形式で属性値を取得できます。

#### 属性アクセスを使わない場合

`属性アクセス`は糖衣構文であり、使用しなくても属性値を取得できます。  
ただし、通常の参照では下記のように冗長になります。
```
(<列挙>の)<要素>の値の<属性>の値 
```
