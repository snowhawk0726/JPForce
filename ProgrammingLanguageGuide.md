...
語尾変化する動詞や形容詞の識別子を算出で定義する場合、通常の定義文では日本語として不自然に見えます。  
以下の形式の**算出定義文**を使うことにより、より日本語らしい記述が可能となります。
...
注１：「さらに、」により多重定義可能  
注２：<算出(取得)の定義内容>は、算出の形式１と同じです。([算出の定義](#算出computation)を参照)  
ただし、ブロック前の予約語`算出`は不要です。  
注３：「こと。」は、省略可能  
...
(2)の様に、ブロック内が１文である場合は、ブロック記号(`【】`)を省略可能

(1)と(2)を、通常の定義文で定義する場合、以下の様になります。
