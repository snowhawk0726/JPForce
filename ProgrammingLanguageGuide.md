...
1. 述語(名詞、動詞、形容詞）として使用する「`足す`、`引く`、`掛ける`、`割る`、`負数`、`等しい`、`小さい`、`大きい`、`ある`、`ない`、`返す`、`返る`、`実行`、`する`、`追加`、`削除`、`繰り返す`、`中止`、`継続`、`写像`、`まとめる`、`絞り込む`、`並び替える`、`含む`、`積む`、`捨てる`、`得る`、`写す`、`代入`、`入れ替える`、`表示`、`改行`、`読む`、`生成`、`設定`、`逆順`、`利用可能`、`初期化`、`識別子一覧`、`ファイル一覧`」
...
注：指定位置が配列の範囲外である場合、エラーとはせず元の配列をそのまま返します。
...
配列内の位置を指定するには、数値または数値オブジェクトを指す識別子を指定します。

```
形式１：<配列>の<数値>に<値>を代入する
形式２：<配列>の<識別子>に<値>を代入する
形式３：<値>を<配列>の<数値>に代入する
形式４：<値>を<配列>の<識別子>に代入する
```
...
注２：後述の[代入文](#代入文assignment-statement---要素への代入設定)を使うことで、同様の結果が得られます。

_`代入`の例：_

```
配列【１、２、３】の１に５を代入する。(配列【１、５、３】)
甲は、配列【１、２、３】。乙は０。甲の乙に５を代入する。(甲は配列【５、２、３】)
```
２番目の例では、識別子乙が０であるため、指定により位置は０となり、甲は代入結果の配列で上書きされます。
...
また、dも、値をしていませんが、cに値「10」が指定されているため、dの値は、**1を足した**「11」になります。
...
注１：「先頭」は「最初」でも良い。「後尾」は「最後」でも良い。
注２：指定位置が配列の範囲外である場合、エラーとはせず元の配列をそのまま返します。
...
形式２：(<識別子１>(と<識別子２>…)に)「値」を(n個)得る。
形式３：(<識別子１>(と<識別子２>…)に)「数値」を(n個)得る。
```



1. <識別子>を指定すると、<識別子>にスタックから値を取得し、代入する。
1. 「値」を指定すると、取得値から(格を除いた)値を取り出す。
