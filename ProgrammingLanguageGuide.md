...
列挙は、**列挙子(enumerator)** と、それに対応づけられた**値（任意のオブジェクト）** の集合です。  

そして、列挙の構成要素である**要素**は、  
（1）列挙子（識別子）  
（2）その列挙子に対応づけられた値（オブジェクト）  
の組（ペア）からなります。
...
さらに、**値**に**数値**を指定した場合は、続く列挙子の値は、1を足した**連続した数値**となります。  
