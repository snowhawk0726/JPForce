...
強制的に外部の識別子を参照する場合は、述語`外部`を使用します。([外部](#外部outer)参照)
...
配列内の位置を指定するには、述語`位置`を用い、直後に数値または数値オブジェクトを指定します。
...
型に依存する式などの構文は、それぞれの型で説明しています。([値と型](#値literalと型type)参照)。また、型に共通する述語、その他の述語については、[述語](#述語predicate)で説明します。
...
同名の識別子がある場合、述語`外部`を指定することにより、呼び元に代入することが可能です。
...
通常、外側の識別子は、同名の識別子がローカルに存在しない場合には、参照・変更が可能ですが、`外部`で指定することにより、明示的に参照・変更が可能になります。  
...
代入文では、`代入`や`設定`、`位置`、`要素`といった述語が不要になります。
...
注：  
外側とローカル辞書の両方に同じ識別子がある場合、代入結果は、ローカル辞書に登録されます。外側の識別子を指定するためには、[述語`外部`](#外部outer)を使用します。
