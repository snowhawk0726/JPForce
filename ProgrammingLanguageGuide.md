...
1. 制御文で使用する「`場合`、`反復`、`間`」
1. 述語(名詞、動詞、形容詞）として使用する「`足す`、`引く`、`掛ける`、`割る`、`等しい`、`小さい`、`大きい`、`ある`、`ない`、`もの`、`正`、`負`、`返す`、`返る`、`実行`、`する`、`追加`、`削除`、`繰り返す`、`中止`、`継続`、`写像`、`まとめる`、`絞り込む`、`並び替える`、`含む`、`入力`、`積む`、`捨てる`、`得る`、`写す`、`空`、`代入`、`入れ替える`、`表示`、`改行`、`読む`、`生成`、`上書き`、`設定`、`かつ`、`または`、`および`、`こと`、`か`、`よって`、`逆順`、`ファイル`、`ファイル一覧`、`識別子`、`識別子一覧`、`位置`、`要素`、`利用可能`、`初期化`」
...
(注：[代入文](#代入文assignment-statement---要素への代入設定)を使うことにより、変更は可能です。)
...
(5) 甲は、辞書【1が「一」、2が「二」、3が「三」】。乙は２。関数【「弍」を甲のキー『乙』に上書きする。】を実行する。(辞書【1が「一」、2が「弍」、3が「三」】)
```

(1)と(2)は代入する対象がリテラルのため、代入された辞書が返ります。((2)は追加)  
(3)から(5)は代入する対象が識別子のため、代入された結果が識別子に代入されます。  
(5)では、関数は呼び元の識別子に代入することはできないので、`上書き`で代入しています。([`上書き`](#上書きoverwrite)参照)
...
指定する位置は、`数値`、もしくは数値を値に持つ`識別子`です。キーに識別子を指定する場合、識別子は二重かぎ括弧`『』`で囲います。

#### `キー`(Key)

[辞書要素のアクセス](#辞書要素のアクセス)で、[`代入`](#辞書に代入)、および[`上書き`](#上書きoverwrite)時に辞書のキーを指定します。  
指定するキーは、`文字列`、`数値`、`真偽値`、もしくは、これらを値に持つ`識別子`です。キーに識別子を指定する場合、識別子は二重かぎ括弧`『』`で囲います。
...
また、識別子が、外側の辞書（関数の呼び元等）にある場合、結果のオブジェクトは、呼び元の辞書に登録されます。（[`上書き`(overwrite)](#上書きoverwrite))  
