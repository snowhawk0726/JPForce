...
1. 制御文で使用する「`場合`、`反復`、`間`」
1. 述語(名詞、動詞、形容詞）として使用する「`足す`、`引く`、`掛ける`、`割る`、`等しい`、`小さい`、`大きい`、`ある`、`ない`、`もの`、`正`、`負`、`返す`、`実行`、`する`、`追加`、`削除`、`繰り返す`、`中止`、`写像`、`まとめる`、`絞り込む`、`並び替える`、`含む`、`入力`、`積む`、`捨てる`、`得る`、`写す`、`空`、`代入`、`入れ替える`、`表示`、`改行`、`読む`、`生成`、`上書き`、`設定`、`以上`、`以下`、`未満`、`かつ`、`または`、`および`、`こと`、`か`、`よって`、`逆順`、`ファイル`、`ファイル一覧`、`識別子`、`識別子一覧`、`位置`、`要素`、`利用可能`、`初期化`」
...
また、特殊な助詞として、個数を表す`個`も同様に助詞として扱います。  
...
1. 記号(注)を含む語
1. 数値で始まる語
1. 予約語「する」「し」「こと」「また」「以上」「以下」「未満」「場合」で終わる語
...
1. 語尾の辞書(連用形 → 終止形)  
   “い”→”う”, "え”→”える", "き”→”く", "ぎ”→”ぐ", "く”→”い", "け”→”ける", "し”→”す", "せ”→”せる", "ち”→”つ", "に”→”ぬ", "び”→”ぶ", "み”→”む", "り”→”る", "れ”→”れる"
1. 語の辞書(連用形 → 終止形)  
   "し”→”する", "て”→”た", "で”→”だ", "あっ”→”ある", "引い”→”引く", "割っ”→”割る", "捨て”→”捨てる", "まとめ”→”まとめる", "絞り込ん"→"絞り込む", "含ん"→"含む", "積ん"→"積む", "見”→”見る", "得”→”得る", "含ま”→”含む"
...
ただし、`個`は、数値の後に続けても単位ではなく「助詞」として扱われます。これは、引数等で<数値>`個`を「個数」として扱い、通常の数値と区別するためです。)
...
述語`以上`、`以下`、`未満`は、上記の通り、「である」(否定形は「でない」)を付ける必要があります。
...
<範囲式>は、数値返す式と助詞`から`,`まで`、および述語`以上`,`以下`,`未満`で構成されます。また、組み合わせにより、下限、上限、上下限の３通りを表現できます。  
`から`と`以上`は下限を表し、`まで`と`以下`と`未満`は上限を表します。上下限を表す場合は、助詞は助詞どうし、述語は述語どうしでのみ組み合わせが可能です。
...
加算は、関数であって、入力がaとb、aとbを足し、返す。(引数aとbを足して返す関数を、識別子「加算」に割り当てる。)
```
...
フィボナッチ数は、関数であって、【入力が数字「数値」であり、本体が、
    ...
    】
】。
1から12まで反復【入力が数字、数字でフィボナッチ数を実行し、表示する】。
...
形式２：<数値>から<数値>まで（<数値>ずつ）反復【入力が<識別子(カウント値)>、<実行処理>】。
形式３：<範囲>を反復【入力が<識別子(カウント値)>、<実行処理>】。
形式４：<配列>を反復【入力が<識別子(値)>、<実行処理>】。
形式５：<辞書>を反復【入力が<識別子(キー)>と<識別子(値)>、<実行処理>】。
```
...
```
(1) 二倍は、関数【入力がx、xに2を掛け、返す】。５を二倍し、二倍する。(20)
(2) 加算は、関数【入力がaとb、aにbを足す】。1に2を加算する。(3)
(3) 5で関数【入力がx、x】を実行する。(5)
```
...
```
適用は、関数【入力がaとbと演算、aとbを演算する】。
加算は、関数【入力がaとb、aにbを足す】。10と2に、加算を適用する。(12)
減算は、関数【入力がaとb、aからbを引く】。10と2に、減算を適用する。(8)
```
...
引数の識別子名、および型と格をチェックすることが可能です。チェックするには、入力の識別子にかぎ括弧`「`と`」`を付けて、以下の様に記述します。
...
```
(1) 加算は、関数【入力がa「数値」とb「数値」、aにbを足す】。1と2の加算する。(3。型のチェックOK)
(2) 加算は、関数【入力がa「と」とb「を」、aにbを足す】。1と2を加算する。(3。格のチェックOK)
(3) 加算は、関数【入力がa「数値に」とb「数値を」、aにbを足す】。1に2を加算する。(3。型と格のチェックOK)
(4) 加算は、関数【入力がa「数値に」とb「数値を」、aにbを足す】。「a」に「b」を加算する。(エラー：入力の型が異なる。入力の型：文字列)
(5) 加算は、関数【入力がa「数値に」とb「数値を」、aにbを足す】。1と2を加算する。(エラー：入力の格が異なる。入力の格：と)
(6) 加算は、関数【入力がa「数値と…」とb「数値を」。aをbと関数【入力が初期値と値、初期値に値を足す】でまとめ、返す】。1と2と3と4と5を加算する。(15)
```

(４)と(５)の例では、引数のシグネチャが異なるためエラーが返っています。(６)の例では、述語`まとめる`(reduce)を使い、1 から 5 までの合計を求めています。引数のシグネチャがチェック形式と一致しているので結果が返っています。
...
```
合成は、関数【入力がa「文字列と…」とb「文字列を」、aの最初にbを足し、返す】。
合成は、さらに、関数【入力がa「数値に」とb「数値を」、aにbを足し、返す】。
合成は、さらに、関数【入力がa「文字列を」とb「文字列に」、bにaを足し、返す】。
※
「あ」と「い」を合成する。(「あい」)
1に2を合成する。(3)
「あ」を「い」に合成する。(「いあ」)
...
```
(1) 加えるは、算出【取得が【入力がa「数値」とb「数値」、aにbを足し、返す】】。
(2) 加えるは、さらに、算出【取得が、さらに、【入力がa「数値に」とb「数値を」、aにbを足し、返す】】。
(3) 加えるは、さらに、算出【
...
計算に使える述語は、`足す`、`掛ける`、`割る`、`引く`、`負数`、`追加`、`削除`、`逆順`です。  
...
```
キュー(FIFO)は、配列。
入れるは、算出で、入力が値、キューに値を追加して上書きする。
取り出すは、算出で、値はキューの先頭。キューが空でない場合、キューの残りをキューに上書きする。値を返す。
3を入れる。
5を入れる。
取り出す。(3)
```
...
```
配列【１、２0、３、４0、５】が、関数【入力が数値、数値が10より大きい】を含む。(真)
辞書【「あ」が１、「い」が２、「う」が30】が、関数【入力がキーと値、値が10より大きい】を含む。(真)
```
...
```
二倍は、関数【入力が数値、数値に２を掛ける】。
配列【１、２、３、４、５】を二倍で写像する。(配列【2、4、6、8、10】)
範囲【1から5まで】を二倍で写像する。(配列【2、4、6、8、10】)
辞書【「あ」が１、「い」が２、「う」が30】を、関数【入力がキーと値、値に2を掛ける】で写像する。(配列【4、２、60】)
```
...
```
配列【１、２0、３、４0、５】を、関数【入力が数値、数値が10より大きい】で絞り込む。(配列【20、40】)
辞書【「あ」が１、「い」が２、「う」が30】を、関数【入力がキーと値、値が10より大きい】で絞り込む。(辞書【「う」が30】)
```
...
```
配列【「あ」、「い」、「う」、「え」、「お」】を、「」と関数【入力が初期値と文字、初期値に文字を足す】でまとめる。(「あいうえお」)
...
配列【３、１、４、２、５】を、関数【入力が甲と乙、甲が乙より大きい】で並べ替える。(配列【５、４、３、２、１】)
```
...
ファイル一覧を反復【入力がファイル名、ファイル名を表示する】。(ファイル一覧を、ファイル名ごとに改行して表示する。)
```
...
識別子一覧で関数【入力がキーと値、キーを表示する】を繰り返す。(識別子一覧を一行ずつ表示する。)
識別子一覧で関数【入力がキーと定義、「\(キー)は、\(定義)」を表示する】を繰り返す。(ローカル辞書の、識別子とその定義を表示する。)
```
